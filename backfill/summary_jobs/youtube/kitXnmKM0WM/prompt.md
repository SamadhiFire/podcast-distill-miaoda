...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/kitXnmKM0WM/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/kitXnmKM0WM/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/kitXnmKM0WM/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:kitXnmKM0WM",
  "platform": "youtube",
  "platform_id": "kitXnmKM0WM",
  "source_id": "yt_the_daily",
  "source_name": "The Daily",
  "category": "新闻 / 时评 / 全球议题",
  "title": "Every Eight Minutes: Uber’s Alarming Sexual Violence Problem",
  "url": "https://www.youtube.com/watch?v=kitXnmKM0WM",
  "published_at": "2025-08-08T14:30:04+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/kitXnmKM0WM/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-09",
  "duration_seconds": 2099,
  "transcript_sha256": "826737893053d5f8d6f8cef5ef7951c61d285c14749d7853cc9b55ab0a1c898d",
  "transcript_text_chars": 27074,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9965269175797999,
    "text_chars": 27074,
    "completed_at": "2026-06-29 08:43:44"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:kitXnmKM0WM
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/UbjAOCzpNWc/transcript.txt`
- 写入结果文件：`backfill/items/youtube/UbjAOCzpNWc/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:UbjAOCzpNWc",
  "platform": "youtube",
  "platform_id": "UbjAOCzpNWc",
  ...
  "published_at": "2025-07-13T11:00:46+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/UbjAOCzpNWc/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-07-14",
  "duration_seconds": 6094,
  "transcript_sha256": "47fdd90a23e70746efa97d0ba1ea04ac5badd44d7cc4fde4f379925f8d491d8f",
  "transcript_text_chars": 106296,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9991073186741057,
    "text_chars": 106296,
    "completed_at": "2026-06-29 07:55:43"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:UbjAOCzpNWc
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/Y_bczlEqtBY/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/Y_bczlEqtBY/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/Y_bczlEqtBY/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:Y_bczlEqtBY",
  "platform": "youtube",
  "platform_id": "Y_bczlEqtBY",
  "source_id": "yt_bloomberg_china",
  "source_name": "Bloomberg The China Show",
  "category": "商业 / 财经 / 投资",
  "title": "Policy Support Boosts China Chip, AI Stocks | The China Show 8/27/2025",
  "url": "https://www.youtube.com/watch?v=Y_bczlEqtBY",
  "published_at": "2025-08-27T05:38:41+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/Y_bczlEqtBY/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-28",
  "duration_seconds": 5514,
  "transcript_sha256": "16183f955be17a47c586c4cf4583eeab02c2beb27044522a8cf1310ec7627acd",
  "transcript_text_chars": 89836,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9996427276024664,
    "text_chars": 89836,
    "completed_at": "2026-06-29 09:35:29"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:Y_bczlEqtBY
```

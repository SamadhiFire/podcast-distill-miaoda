...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/K05Dh-QjM8c/transcript.txt`
- 写入结果文件：`backfill/items/youtube/K05Dh-QjM8c/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:K05Dh-QjM8c",
  "platform": "youtube",
  "platform_id": "K05Dh-QjM8c",
  ...
  "published_at": "2026-05-06T19:03:31+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/K05Dh-QjM8c/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-05-07",
  "duration_seconds": 638,
  "transcript_sha256": "e1ce0394c8c5d0609217181a01f033ecbf7349011502807425ddb92bb0d823aa",
  "transcript_text_chars": 9265,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9917507836990596,
    "text_chars": 9265,
    "completed_at": "2026-06-29 13:42:50"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:K05Dh-QjM8c
```

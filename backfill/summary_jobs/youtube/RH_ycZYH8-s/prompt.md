...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/RH_ycZYH8-s/transcript.txt`
- 写入结果文件：`backfill/items/youtube/RH_ycZYH8-s/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:RH_ycZYH8-s",
  "platform": "youtube",
  "platform_id": "RH_ycZYH8-s",
  ...
  "published_at": "2025-10-31T15:30:30+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/RH_ycZYH8-s/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-11-01",
  "duration_seconds": 5484,
  "transcript_sha256": "7f443afaa9266fb288a09e7d25db07d760fd8766d36b7c35eec6589545577d33",
  "transcript_text_chars": 85674,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9993141867250181,
    "text_chars": 85674,
    "completed_at": "2026-06-29 11:31:17"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:RH_ycZYH8-s
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/aNVUB8sal6M/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/aNVUB8sal6M/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/aNVUB8sal6M/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:aNVUB8sal6M",
  "platform": "youtube",
  "platform_id": "aNVUB8sal6M",
  "source_id": "yt_wsj_bold_names",
  "source_name": "WSJ Bold Names",
  "category": "新闻 / 时评 / 全球议题",
  "title": "How Tubi Is Coming for Netflix and YouTube in the New Streaming Wars",
  "url": "https://www.youtube.com/watch?v=aNVUB8sal6M",
  "published_at": "2025-08-01T15:44:57+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/aNVUB8sal6M/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-02",
  "duration_seconds": 2670,
  "transcript_sha256": "54cad74ddbf035c167755ea2b70b5fdbb1eb8c33bf269b0186686fe3bc411dc6",
  "transcript_text_chars": 42330,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9961947565543072,
    "text_chars": 42330,
    "completed_at": "2026-06-29 08:27:10"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:aNVUB8sal6M
```

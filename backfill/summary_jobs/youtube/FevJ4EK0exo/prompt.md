...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/FevJ4EK0exo/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/FevJ4EK0exo/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/FevJ4EK0exo/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:FevJ4EK0exo",
  "platform": "youtube",
  "platform_id": "FevJ4EK0exo",
  "source_id": "yt_odd_lots_audio",
  "source_name": "Odd Lots Audio",
  "category": "商业 / 财经 / 投资",
  "title": "Mary Daly on Why Alaska Is a Leading Indicator for the US Economy | Odd Lots",
  "url": "https://www.youtube.com/watch?v=FevJ4EK0exo",
  "published_at": "2025-08-08T14:12:21+00:00",
  "report_date": "2025-08-09",
  "duration_seconds": 4395,
  "language": "en"
}
```
...
把下面 JSON 写入 `backfill/items/youtube/FevJ4EK0exo/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-09",
  "duration_seconds": 4395,
  "transcript_sha256": "f86671b99c6995e2054ff05ab06a8809fbba7175cef74b31a44ee40d28db6525",
  "transcript_text_chars": 78437,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9998179749715586,
    "text_chars": 78437,
    "completed_at": "2026-06-29 08:43:18"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:FevJ4EK0exo
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/vagyIcmIGOQ/transcript.txt`
- 写入结果文件：`backfill/items/youtube/vagyIcmIGOQ/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:vagyIcmIGOQ",
  "platform": "youtube",
  "platform_id": "vagyIcmIGOQ",
  ...
  "published_at": "2025-07-12T17:15:26+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/vagyIcmIGOQ/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-07-13",
  "duration_seconds": 22128,
  "transcript_sha256": "7b525819fa611855ddf2de074b5360676a25f4528e906623b5fa9df9571a49c4",
  "transcript_text_chars": 344506,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.999533622559653,
    "text_chars": 344506,
    "completed_at": "2026-06-29 07:55:11"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:vagyIcmIGOQ
```

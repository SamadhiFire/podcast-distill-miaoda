...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/hIQjOCNidb0/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/hIQjOCNidb0/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/hIQjOCNidb0/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:hIQjOCNidb0",
  "platform": "youtube",
  "platform_id": "hIQjOCNidb0",
  "source_id": "yt_no_priors",
  "source_name": "No Priors Podcast",
  "category": "科技 / AI / VC",
  "title": "No Priors Ep. 129 | With U.S. Under Secretary of State (E) Designate Jacob Helberg",
  "url": "https://www.youtube.com/watch?v=hIQjOCNidb0",
  "published_at": "2025-08-28T10:01:21+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/hIQjOCNidb0/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-29",
  "duration_seconds": 2454,
  "transcript_sha256": "6d0f18dd1be184026b3168cf62dafd56fcb960dda3f0f01252384ac9ab58d8a3",
  "transcript_text_chars": 37043,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9983700081499592,
    "text_chars": 37043,
    "completed_at": "2026-06-29 09:37:57"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:hIQjOCNidb0
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/hb-vhlP4zPI/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/hb-vhlP4zPI/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/hb-vhlP4zPI/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:hb-vhlP4zPI",
  "platform": "youtube",
  "platform_id": "hb-vhlP4zPI",
  "source_id": "yt_bloomberg_surveillance",
  "source_name": "Bloomberg Surveillance Full Shows",
  "category": "商业 / 财经 / 投资",
  "title": "Bloomberg Surveillance 8/8/2025",
  "url": "https://www.youtube.com/watch?v=hb-vhlP4zPI",
  "published_at": "2025-08-08T15:53:58+00:00",
  "report_date": "2025-08-09",
  "duration_seconds": 8689,
  "language": "en"
}
```
...
把下面 JSON 写入 `backfill/items/youtube/hb-vhlP4zPI/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-09",
  "duration_seconds": 8689,
  "transcript_sha256": "9f1584faad0addb62cb52476e17d456d22753182f5335e56ea3f9ebd08dbf3ff",
  "transcript_text_chars": 153684,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9993324893543561,
    "text_chars": 153684,
    "completed_at": "2026-06-29 08:44:11"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:hb-vhlP4zPI
```

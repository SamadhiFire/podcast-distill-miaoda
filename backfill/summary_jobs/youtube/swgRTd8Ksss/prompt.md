...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/swgRTd8Ksss/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/swgRTd8Ksss/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/swgRTd8Ksss/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:swgRTd8Ksss",
  "platform": "youtube",
  "platform_id": "swgRTd8Ksss",
  "source_id": "yt_business_breakdowns",
  "source_name": "Business Breakdowns",
  "category": "商业 / 财经 / 投资",
  "title": "Agilent: Back To The Lab - [Business Breakdowns, EP.223]",
  "url": "https://www.youtube.com/watch?v=swgRTd8Ksss",
  "published_at": "2025-08-17T20:59:54+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/swgRTd8Ksss/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "extraction": {
    ...
    "text_chars": 44899,
    "completed_at": "2026-06-29 09:02:22"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:swgRTd8Ksss
```

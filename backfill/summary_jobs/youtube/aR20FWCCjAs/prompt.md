...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/aR20FWCCjAs/transcript.txt`
- 写入结果文件：`backfill/items/youtube/aR20FWCCjAs/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:aR20FWCCjAs",
  "platform": "youtube",
  "platform_id": "aR20FWCCjAs",
  ...
  "published_at": "2025-11-25T17:29:11+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/aR20FWCCjAs/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-11-26",
  "duration_seconds": 5764,
  "transcript_sha256": "496aa4c14875634e79278c6565182bf838d1af5e1671caf8031a345b0bd91452",
  "transcript_text_chars": 86660,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9993199167244968,
    "text_chars": 86660,
    "completed_at": "2026-06-29 11:54:05"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:aR20FWCCjAs
```

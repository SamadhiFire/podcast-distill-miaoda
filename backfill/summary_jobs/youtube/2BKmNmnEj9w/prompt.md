...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/2BKmNmnEj9w/transcript.txt`
- 写入结果文件：`backfill/items/youtube/2BKmNmnEj9w/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:2BKmNmnEj9w",
  "platform": "youtube",
  "platform_id": "2BKmNmnEj9w",
  ...
  "published_at": "2025-10-05T11:00:05+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/2BKmNmnEj9w/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-10-06",
  "duration_seconds": 5125,
  "transcript_sha256": "2798c94315b26041ac4729fe6e3e398da1d79fd18b862ffe033b109129fbf2ab",
  "transcript_text_chars": 93021,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9990554146341463,
    "text_chars": 93021,
    "completed_at": "2026-06-29 11:10:59"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:2BKmNmnEj9w
```

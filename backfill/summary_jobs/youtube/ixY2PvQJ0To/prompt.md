...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/ixY2PvQJ0To/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/ixY2PvQJ0To/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/ixY2PvQJ0To/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:ixY2PvQJ0To",
  "platform": "youtube",
  "platform_id": "ixY2PvQJ0To",
  ...
  "published_at": "2025-08-09T11:01:24+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/ixY2PvQJ0To/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-10",
  "duration_seconds": 5738,
  "transcript_sha256": "a18378667cb5b68ebd33c0b5d4aa322a328d28fc3807a58a59b2b1aa79e9d1b8",
  "transcript_text_chars": 106232,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9991355873126525,
    "text_chars": 106232,
    "completed_at": "2026-06-29 08:45:01"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:ixY2PvQJ0To
```

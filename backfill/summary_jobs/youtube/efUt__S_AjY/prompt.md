...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/efUt__S_AjY/transcript.txt`
- 写入结果文件：`backfill/items/youtube/efUt__S_AjY/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:efUt__S_AjY",
  "platform": "youtube",
  "platform_id": "efUt__S_AjY",
  ...
  "published_at": "2026-02-19T11:01:09+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/efUt__S_AjY/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-02-20",
  "duration_seconds": 2442,
  "transcript_sha256": "2ea3729bc12529675d6b8b425f50a8db56abb17da8a29cf7f1526fddc0bcc4fc",
  "transcript_text_chars": 44378,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9981162981162982,
    "text_chars": 44378,
    "completed_at": "2026-06-29 12:48:39"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:efUt__S_AjY
```

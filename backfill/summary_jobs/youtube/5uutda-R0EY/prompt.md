...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/5uutda-R0EY/transcript.txt`
- 写入结果文件：`backfill/items/youtube/5uutda-R0EY/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:5uutda-R0EY",
  "platform": "youtube",
  "platform_id": "5uutda-R0EY",
  ...
  "published_at": "2025-11-11T10:00:10+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/5uutda-R0EY/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-11-12",
  "duration_seconds": 2619,
  "transcript_sha256": "dce7ab9adf47c43cc08526d5e8982ee64cf16ffcb035edaf35e68e4608beefff",
  "transcript_text_chars": 44176,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9931386025200459,
    "text_chars": 44176,
    "completed_at": "2026-06-29 11:40:02"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:5uutda-R0EY
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/vJswjGMvHLs/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/vJswjGMvHLs/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/vJswjGMvHLs/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:vJswjGMvHLs",
  "platform": "youtube",
  "platform_id": "vJswjGMvHLs",
  "source_id": "yt_the_interview_nyt",
  "source_name": "The Interview - New York Times",
  "category": "新闻 / 时评 / 全球议题",
  "title": "The ADL’s CEO: ‘Anti-Zionism is an ideology of nihilism’ | The Interview",
  "url": "https://www.youtube.com/watch?v=vJswjGMvHLs",
  "published_at": "2025-08-09T09:01:24+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/vJswjGMvHLs/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-10",
  "duration_seconds": 3758,
  "transcript_sha256": "6bb935ca7c4b8494d8faa2cf748f4747345d92d2d2aaabb76f3947dc9f532e9d",
  "transcript_text_chars": 55852,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9989089941458222,
    "text_chars": 55852,
    "completed_at": "2026-06-29 08:44:34"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:vJswjGMvHLs
```

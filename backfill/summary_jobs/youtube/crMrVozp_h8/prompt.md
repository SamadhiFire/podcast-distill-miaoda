...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/crMrVozp_h8/transcript.txt`
- 写入结果文件：`backfill/items/youtube/crMrVozp_h8/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:crMrVozp_h8",
  "platform": "youtube",
  "platform_id": "crMrVozp_h8",
  ...
  "published_at": "2025-07-17T11:01:09+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/crMrVozp_h8/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-07-18",
  "duration_seconds": 5697,
  "transcript_sha256": "acaacf20bcdb38812b5ccba0ba75e494e5e5b7848a84abe2f1cde9136a3f4cf5",
  "transcript_text_chars": 101875,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9991502545199227,
    "text_chars": 101875,
    "completed_at": "2026-06-29 08:03:35"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:crMrVozp_h8
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/cgj3o-NPApg/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/cgj3o-NPApg/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/cgj3o-NPApg/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:cgj3o-NPApg",
  "platform": "youtube",
  "platform_id": "cgj3o-NPApg",
  "source_id": "yt_bloomberg_surveillance",
  "source_name": "Bloomberg Surveillance Full Shows",
  "category": "商业 / 财经 / 投资",
  "title": "Bloomberg Surveillance 8/11/2025",
  "url": "https://www.youtube.com/watch?v=cgj3o-NPApg",
  "published_at": "2025-08-11T16:05:28+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/cgj3o-NPApg/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-12",
  "duration_seconds": 8656,
  "transcript_sha256": "38bebb67b1d94e96d7655219ee1c51770bf9aff5f7d7e7598eb68cd036ec8a0d",
  "transcript_text_chars": 155025,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9994085027726431,
    "text_chars": 155025,
    "completed_at": "2026-06-29 08:47:41"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:cgj3o-NPApg
```

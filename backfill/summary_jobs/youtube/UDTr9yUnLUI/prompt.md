...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/UDTr9yUnLUI/transcript.txt`
- 写入结果文件：`backfill/items/youtube/UDTr9yUnLUI/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:UDTr9yUnLUI",
  "platform": "youtube",
  "platform_id": "UDTr9yUnLUI",
  ...
  "published_at": "2026-05-26T12:00:19+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/UDTr9yUnLUI/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-05-27",
  "duration_seconds": 2734,
  "transcript_sha256": "e23ce1ef21d94edde7cc5d7cb8a2e66f089ef40014112f8b818919bb00e4af47",
  "transcript_text_chars": 49661,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9976046086320409,
    "text_chars": 49661,
    "completed_at": "2026-06-29 13:57:45"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:UDTr9yUnLUI
```

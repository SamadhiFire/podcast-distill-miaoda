...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/WdbgNC80PMw/transcript.txt`
- 写入结果文件：`backfill/items/youtube/WdbgNC80PMw/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:WdbgNC80PMw",
  "platform": "youtube",
  "platform_id": "WdbgNC80PMw",
  ...
  "published_at": "2025-11-14T00:16:05+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/WdbgNC80PMw/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-11-15",
  "duration_seconds": 4462,
  "transcript_sha256": "bcde012b9f4bed7c1faedc363935565ffd11d28a271774aa0cc8af65d6f12c58",
  "transcript_text_chars": 82484,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9914657104437472,
    "text_chars": 82484,
    "completed_at": "2026-06-29 11:43:56"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:WdbgNC80PMw
```

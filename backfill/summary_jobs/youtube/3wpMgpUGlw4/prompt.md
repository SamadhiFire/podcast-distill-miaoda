...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/3wpMgpUGlw4/transcript.txt`
- 写入结果文件：`backfill/items/youtube/3wpMgpUGlw4/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:3wpMgpUGlw4",
  "platform": "youtube",
  "platform_id": "3wpMgpUGlw4",
...
把下面 JSON 写入 `backfill/items/youtube/3wpMgpUGlw4/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-02-20",
  "duration_seconds": 2683,
  "transcript_sha256": "8ab1e3e10239d81d7874f8d508f2401fb0886bfa3e373bf9e5c0de06418df053",
  "transcript_text_chars": 39692,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9985016772269847,
    "text_chars": 39692,
    "completed_at": "2026-06-29 12:49:00"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:3wpMgpUGlw4
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/7CtBsylelBo/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/7CtBsylelBo/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/7CtBsylelBo/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:7CtBsylelBo",
  "platform": "youtube",
  "platform_id": "7CtBsylelBo",
  "source_id": "yt_bloomberg_originals",
  "source_name": "Bloomberg Originals",
  "category": "商业 / 财经 / 投资",
  "title": "Meghan, the Duchess of Sussex, on Life and Business Beyond the Royal Spotlight | The Circuit",
  "url": "https://www.youtube.com/watch?v=7CtBsylelBo",
  "published_at": "2025-08-26T08:00:14+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/7CtBsylelBo/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-27",
  "duration_seconds": 2793,
  "transcript_sha256": "6343a59fbdd943aa22a9601bed7e4a42266fb6123f248c58bd124621d3d05118",
  "transcript_text_chars": 46113,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9976942355889724,
    "text_chars": 46113,
    "completed_at": "2026-06-29 09:31:39"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:7CtBsylelBo
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/QhAftC_zFr8/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/QhAftC_zFr8/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/QhAftC_zFr8/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:QhAftC_zFr8",
  "platform": "youtube",
  "platform_id": "QhAftC_zFr8",
  "source_id": "yt_acquired",
  "source_name": "Acquired Podcast Full Episodes",
  "category": "商业 / 财经 / 投资",
  "title": "Google Part II: Alphabet (Audio)",
  "url": "https://www.youtube.com/watch?v=QhAftC_zFr8",
  "published_at": "2025-08-26T10:45:06+00:00",
  "report_date": "2025-08-27",
  "duration_seconds": 15093,
  "language": "en"
}
```
...
把下面 JSON 写入 `backfill/items/youtube/QhAftC_zFr8/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-27",
  "duration_seconds": 15093,
  "transcript_sha256": "8fd3d91c42f36cf4175821e0b0b395fa82dc710d3411cbbe50686aa9034b65a1",
  "transcript_text_chars": 240024,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9993612933147817,
    "text_chars": 240024,
    "completed_at": "2026-06-29 09:33:46"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:QhAftC_zFr8
```

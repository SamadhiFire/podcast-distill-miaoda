...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/96jN2OCOfLs/transcript.txt`
- 写入结果文件：`backfill/items/youtube/96jN2OCOfLs/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:96jN2OCOfLs",
  "platform": "youtube",
  "platform_id": "96jN2OCOfLs",
  ...
  "published_at": "2026-04-29T15:21:18+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/96jN2OCOfLs/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-04-30",
  "duration_seconds": 1789,
  "transcript_sha256": "81bf62c9bf92cbd554419257b67e99d8b9578fa80991ea01e1bbed1b5dad0cb7",
  "transcript_text_chars": 32465,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9966070430408049,
    "text_chars": 32465,
    "completed_at": "2026-06-29 13:35:06"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:96jN2OCOfLs
```

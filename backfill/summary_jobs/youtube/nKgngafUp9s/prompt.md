...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/nKgngafUp9s/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/nKgngafUp9s/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/nKgngafUp9s/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:nKgngafUp9s",
  "platform": "youtube",
  "platform_id": "nKgngafUp9s",
  "source_id": "yt_wsj_take_on_week",
  "source_name": "WSJ Take On the Week",
  "category": "新闻 / 时评 / 全球议题",
  "title": "Why This Economist Says We’re in a Housing Market Recession | WSJ’s Take On the Week",
  "url": "https://www.youtube.com/watch?v=nKgngafUp9s",
  "published_at": "2025-08-17T10:01:11+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/nKgngafUp9s/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-18",
  "duration_seconds": 1419,
  "transcript_sha256": "e3a8ba7ed8211ba2a013458f3967a4b957077dc22f3b7f15d9ece7de48463395",
  "transcript_text_chars": 25533,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9951021846370683,
    "text_chars": 25533,
    "completed_at": "2026-06-29 09:00:38"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:nKgngafUp9s
```

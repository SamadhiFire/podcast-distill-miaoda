...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/0GMWxuYuxJI/transcript.txt`
- 写入结果文件：`backfill/items/youtube/0GMWxuYuxJI/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:0GMWxuYuxJI",
  "platform": "youtube",
  "platform_id": "0GMWxuYuxJI",
  ...
  "published_at": "2025-10-10T17:11:13+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/0GMWxuYuxJI/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-10-11",
  "duration_seconds": 4853,
  "transcript_sha256": "8d26d106706d7bbec2d7f71f89bebeb96167ea13d046c5231182a68e0565e5bb",
  "transcript_text_chars": 86781,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9993818256748404,
    "text_chars": 86781,
    "completed_at": "2026-06-29 11:14:58"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:0GMWxuYuxJI
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/YAJMnZ8QkmY/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/YAJMnZ8QkmY/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/YAJMnZ8QkmY/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:YAJMnZ8QkmY",
  "platform": "youtube",
  "platform_id": "YAJMnZ8QkmY",
  "source_id": "yt_interesting_times",
  "source_name": "Interesting Times with Ross Douthat",
  "category": "新闻 / 时评 / 全球议题",
  "title": "Silicon Valley Wants to Optimize Your Children’s Genes | Interesting Times with Ross Douthat",
  "url": "https://www.youtube.com/watch?v=YAJMnZ8QkmY",
  "published_at": "2025-08-07T14:00:54+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/YAJMnZ8QkmY/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-08",
  "duration_seconds": 3984,
  "transcript_sha256": "b62fdde7af281cb09941c8ae2e040066f2b701c43ea27bc625fa25fc77e09158",
  "transcript_text_chars": 69174,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.996636546184739,
    "text_chars": 69174,
    "completed_at": "2026-06-29 08:39:46"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:YAJMnZ8QkmY
```

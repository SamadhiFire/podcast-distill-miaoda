...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/U1H1Ob7jk8Q/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/U1H1Ob7jk8Q/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/U1H1Ob7jk8Q/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:U1H1Ob7jk8Q",
  "platform": "youtube",
  "platform_id": "U1H1Ob7jk8Q",
  ...
  "published_at": "2025-07-31T21:10:47+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/U1H1Ob7jk8Q/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-01",
  "duration_seconds": 16202,
  "transcript_sha256": "fd50808aea810ea28c3c653c41174d594634814ded28373bb4d66cebd4ca2dba",
  "transcript_text_chars": 226761,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9993432292309592,
    "text_chars": 226761,
    "completed_at": "2026-06-29 08:25:31"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:U1H1Ob7jk8Q
```

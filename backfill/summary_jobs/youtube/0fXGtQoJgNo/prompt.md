...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/0fXGtQoJgNo/transcript.txt`
- 写入结果文件：`backfill/items/youtube/0fXGtQoJgNo/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:0fXGtQoJgNo",
  "platform": "youtube",
  "platform_id": "0fXGtQoJgNo",
  ...
  "published_at": "2026-02-16T15:00:05+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/0fXGtQoJgNo/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-02-17",
  "duration_seconds": 1771,
  "transcript_sha256": "44b4a743264d6cb531602a176f065922c6ce43212da6576f1290fd6e0a4f4788",
  "transcript_text_chars": 26824,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9989836250705816,
    "text_chars": 26824,
    "completed_at": "2026-06-29 12:47:31"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:0fXGtQoJgNo
```

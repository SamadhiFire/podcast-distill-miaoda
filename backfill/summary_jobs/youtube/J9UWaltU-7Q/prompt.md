...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/J9UWaltU-7Q/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/J9UWaltU-7Q/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/J9UWaltU-7Q/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:J9UWaltU-7Q",
  "platform": "youtube",
  "platform_id": "J9UWaltU-7Q",
  ...
  "published_at": "2025-08-28T11:00:36+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/J9UWaltU-7Q/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-29",
  "duration_seconds": 3431,
  "transcript_sha256": "abfe6651a2e79628b2190c61e057b45e939bd0ecf443dc709015bf76ec76a380",
  "transcript_text_chars": 58712,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9985426989215972,
    "text_chars": 58712,
    "completed_at": "2026-06-29 09:38:20"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:J9UWaltU-7Q
```

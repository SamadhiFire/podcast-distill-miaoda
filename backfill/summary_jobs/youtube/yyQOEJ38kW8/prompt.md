...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/yyQOEJ38kW8/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/yyQOEJ38kW8/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/yyQOEJ38kW8/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:yyQOEJ38kW8",
  "platform": "youtube",
  "platform_id": "yyQOEJ38kW8",
  "source_id": "yt_odd_lots_audio",
  "source_name": "Odd Lots Audio",
  "category": "商业 / 财经 / 投资",
  "title": "Liz Truss on the 'Doom Loop' Engulfing the UK Economy | Odd Lots",
  "url": "https://www.youtube.com/watch?v=yyQOEJ38kW8",
  "published_at": "2025-08-28T19:17:49+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/yyQOEJ38kW8/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-29",
  "duration_seconds": 3503,
  "transcript_sha256": "e7d2a9cf1e00e07512d718fa39211852ef3552966d096dfeffcbed9633c971b5",
  "transcript_text_chars": 52354,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.997861832714816,
    "text_chars": 52354,
    "completed_at": "2026-06-29 09:40:12"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:yyQOEJ38kW8
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/_bBRVNkAfkQ/transcript.txt`
- 写入结果文件：`backfill/items/youtube/_bBRVNkAfkQ/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:_bBRVNkAfkQ",
  "platform": "youtube",
  "platform_id": "_bBRVNkAfkQ",
  ...
  "published_at": "2025-12-12T20:01:08+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/_bBRVNkAfkQ/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-12-13",
  "duration_seconds": 7513,
  "transcript_sha256": "cfabfec3d1d6f456203a19a63178f6a50bb5aeed999d667c0d09c9d54acd3a08",
  "transcript_text_chars": 110742,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9984292559563422,
    "text_chars": 110742,
    "completed_at": "2026-06-29 12:07:16"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:_bBRVNkAfkQ
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/0XNkUdzxiZI/transcript.txt`
- 写入结果文件：`backfill/items/youtube/0XNkUdzxiZI/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:0XNkUdzxiZI",
  "platform": "youtube",
  "platform_id": "0XNkUdzxiZI",
  ...
  "published_at": "2026-02-08T13:30:02+00:00",
  "report_date": "2026-02-09",
  "duration_seconds": 6151,
  "language": "en"
}
```
...
把下面 JSON 写入 `backfill/items/youtube/0XNkUdzxiZI/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-02-09",
  "duration_seconds": 6151,
  "transcript_sha256": "e2df8a030335f3f6c9af93de6bacabe32840da606341619659934eb5e661c07a",
  "transcript_text_chars": 98493,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9991220939684605,
    "text_chars": 98493,
    "completed_at": "2026-06-29 12:41:10"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:0XNkUdzxiZI
```

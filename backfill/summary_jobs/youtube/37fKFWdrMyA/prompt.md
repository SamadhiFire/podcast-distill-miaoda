...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/37fKFWdrMyA/transcript.txt`
- 写入结果文件：`backfill/items/youtube/37fKFWdrMyA/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:37fKFWdrMyA",
  "platform": "youtube",
  "platform_id": "37fKFWdrMyA",
  ...
  "published_at": "2025-11-09T13:30:03+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/37fKFWdrMyA/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-11-10",
  "duration_seconds": 4896,
  "transcript_sha256": "57d738390181bfcdea053fa7c765333c7dd1ec1a50996aa1a364020afefd6165",
  "transcript_text_chars": 83528,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.99890522875817,
    "text_chars": 83528,
    "completed_at": "2026-06-29 11:37:50"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:37fKFWdrMyA
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/bBS93A0BeNI/transcript.txt`
- 写入结果文件：`backfill/items/youtube/bBS93A0BeNI/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:bBS93A0BeNI",
  "platform": "youtube",
  "platform_id": "bBS93A0BeNI",
  ...
  "published_at": "2026-04-30T17:27:37+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/bBS93A0BeNI/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-05-01",
  "duration_seconds": 1707,
  "transcript_sha256": "79263dce737485adc186f4d5f6dcd580da3fd2e8644adb75ca17a70796b94161",
  "transcript_text_chars": 30845,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.996900410076157,
    "text_chars": 30845,
    "completed_at": "2026-06-29 13:36:57"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:bBS93A0BeNI
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/ts9ZvlkeWGs/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/ts9ZvlkeWGs/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/ts9ZvlkeWGs/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:ts9ZvlkeWGs",
  "platform": "youtube",
  "platform_id": "ts9ZvlkeWGs",
  "source_id": "yt_lennys_podcast",
  "source_name": "Lenny‘s Podcast",
  "category": "产品 / 创业 / 管理",
  "title": "The one question that saves product careers | Matt LeMay",
  "url": "https://www.youtube.com/watch?v=ts9ZvlkeWGs",
  "published_at": "2025-08-14T11:01:32+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/ts9ZvlkeWGs/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-15",
  "duration_seconds": 5529,
  "transcript_sha256": "7912f754954dba87a815ce41c2ae367773068f428f9ab149d2da8ab85c1b7f28",
  "transcript_text_chars": 92125,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.999124434798336,
    "text_chars": 92125,
    "completed_at": "2026-06-29 08:54:50"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:ts9ZvlkeWGs
```

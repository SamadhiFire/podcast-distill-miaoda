...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/21EYKqUsPfg/transcript.txt`
- 写入结果文件：`backfill/items/youtube/21EYKqUsPfg/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:21EYKqUsPfg",
  "platform": "youtube",
  "platform_id": "21EYKqUsPfg",
  ...
  "published_at": "2025-09-26T16:01:25+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/21EYKqUsPfg/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-09-27",
  "duration_seconds": 4029,
  "transcript_sha256": "9f3660d6da087cd7e4383bac863114710a00e78e24834b841b39af34a8477784",
  "transcript_text_chars": 64350,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9996326631918591,
    "text_chars": 64350,
    "completed_at": "2026-06-29 11:05:45"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:21EYKqUsPfg
```

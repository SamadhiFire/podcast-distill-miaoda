...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/061mrtLveDE/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/061mrtLveDE/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/061mrtLveDE/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:061mrtLveDE",
  "platform": "youtube",
  "platform_id": "061mrtLveDE",
  "source_id": "yt_odd_lots_audio",
  "source_name": "Odd Lots Audio",
  "category": "商业 / 财经 / 投资",
  "title": "What an Alaskan Furniture Company Tells Us About Tariffs | Odd Lots",
  "url": "https://www.youtube.com/watch?v=061mrtLveDE",
  "published_at": "2025-08-11T13:36:58+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/061mrtLveDE/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-12",
  "duration_seconds": 2077,
  "transcript_sha256": "f6f2e6b6f01fefce2a3a571de6ce72d11e299dfc9b54a6077a660d1ff6b9bb30",
  "transcript_text_chars": 33450,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9904044294655754,
    "text_chars": 33450,
    "completed_at": "2026-06-29 08:46:49"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:061mrtLveDE
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/GYWmoyUUHoI/transcript.txt`
- 写入结果文件：`backfill/items/youtube/GYWmoyUUHoI/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:GYWmoyUUHoI",
  "platform": "youtube",
  "platform_id": "GYWmoyUUHoI",
  ...
  "published_at": "2026-01-17T17:00:00+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/GYWmoyUUHoI/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-01-18",
  "duration_seconds": 853,
  "transcript_sha256": "fe99af62f6c63de943affe0b11a35801a9c5a7c6102d36028913cfa2ad4a6544",
  "transcript_text_chars": 15008,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9996717467760844,
    "text_chars": 15008,
    "completed_at": "2026-06-29 12:27:25"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:GYWmoyUUHoI
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/JYaFlTtAmKc/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/JYaFlTtAmKc/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/JYaFlTtAmKc/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:JYaFlTtAmKc",
  "platform": "youtube",
  "platform_id": "JYaFlTtAmKc",
  "source_id": "yt_odd_lots_audio",
  "source_name": "Odd Lots Audio",
  "category": "商业 / 财经 / 投资",
  "title": "Housing Is a Problem Even in a State With Declining Population | Odd Lots",
  "url": "https://www.youtube.com/watch?v=JYaFlTtAmKc",
  "published_at": "2025-08-18T16:34:37+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/JYaFlTtAmKc/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-19",
  "duration_seconds": 3142,
  "transcript_sha256": "8d43e12bbbd7a8dae19ef9a753e1fa8dbef512f509a2bc7fb53ef11db2ec255c",
  "transcript_text_chars": 55302,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9933927434754932,
    "text_chars": 55302,
    "completed_at": "2026-06-29 09:10:35"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:JYaFlTtAmKc
```

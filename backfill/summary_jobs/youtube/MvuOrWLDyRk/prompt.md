...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/MvuOrWLDyRk/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/MvuOrWLDyRk/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/MvuOrWLDyRk/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:MvuOrWLDyRk",
  "platform": "youtube",
  "platform_id": "MvuOrWLDyRk",
  "source_id": "yt_cannonball",
  "source_name": "Cannonball with Wesley Morris",
  "category": "文化 / 社会 / 人文",
  "title": "‘And Just Like That...’ Is Over Too Soon. We Blame Y‘all. | Cannonball with Wesley Morris",
  "url": "https://www.youtube.com/watch?v=MvuOrWLDyRk",
  "published_at": "2025-08-14T12:00:44+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/MvuOrWLDyRk/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-15",
  "duration_seconds": 3003,
  "transcript_sha256": "977f7ca7c5a5c35e823c80d366ab036defb492329a42c3faa32b4138c59733b0",
  "transcript_text_chars": 42583,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9965734265734266,
    "text_chars": 42583,
    "completed_at": "2026-06-29 08:55:17"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:MvuOrWLDyRk
```

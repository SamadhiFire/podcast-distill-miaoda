...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/KxIeJjEGLdo/transcript.txt`
- 写入结果文件：`backfill/items/youtube/KxIeJjEGLdo/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:KxIeJjEGLdo",
  "platform": "youtube",
  "platform_id": "KxIeJjEGLdo",
  ...
  "published_at": "2025-07-25T17:26:23+00:00",
  "report_date": "2025-07-26",
  "duration_seconds": 6982,
  "language": "en"
}
```
...
把下面 JSON 写入 `backfill/items/youtube/KxIeJjEGLdo/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-07-26",
  "duration_seconds": 6982,
  "transcript_sha256": "d4f43f81b00e811831cef3798860cca379046e6ee1b1c93cb0f030b7063ef2a5",
  "transcript_text_chars": 111237,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9993811228874249,
    "text_chars": 111237,
    "completed_at": "2026-06-29 08:17:22"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:KxIeJjEGLdo
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/DPe_srf0GlI/transcript.txt`
- 写入结果文件：`backfill/items/youtube/DPe_srf0GlI/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:DPe_srf0GlI",
  "platform": "youtube",
  "platform_id": "DPe_srf0GlI",
  ...
  "published_at": "2026-05-22T15:30:06+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/DPe_srf0GlI/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-05-23",
  "duration_seconds": 1868,
  "transcript_sha256": "47de32cc32a2ac945d4bb6e566248d8a0d30848784356aad486f42067669d88d",
  "transcript_text_chars": 32884,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9990364025695931,
    "text_chars": 32884,
    "completed_at": "2026-06-29 13:56:22"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:DPe_srf0GlI
```

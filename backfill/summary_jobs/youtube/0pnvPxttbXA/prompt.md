...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/0pnvPxttbXA/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/0pnvPxttbXA/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/0pnvPxttbXA/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:0pnvPxttbXA",
  "platform": "youtube",
  "platform_id": "0pnvPxttbXA",
  "source_id": "yt_the_daily",
  "source_name": "The Daily",
  "category": "新闻 / 时评 / 全球议题",
  "title": "Trump Sends the National Guard Into Washington, D.C.",
  "url": "https://www.youtube.com/watch?v=0pnvPxttbXA",
  "published_at": "2025-08-12T14:20:16+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/0pnvPxttbXA/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-13",
  "duration_seconds": 1429,
  "transcript_sha256": "1dec17d9c1fdf7aa93ecfbd5a72ca83bed51d9d8b4af7ca7d2f8ea964bac8f0e",
  "transcript_text_chars": 19959,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9969769069279216,
    "text_chars": 19959,
    "completed_at": "2026-06-29 08:49:02"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:0pnvPxttbXA
```

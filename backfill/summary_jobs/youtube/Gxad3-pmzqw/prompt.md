...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/Gxad3-pmzqw/transcript.txt`
- 写入结果文件：`backfill/items/youtube/Gxad3-pmzqw/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:Gxad3-pmzqw",
  "platform": "youtube",
  "platform_id": "Gxad3-pmzqw",
  ...
  "published_at": "2025-07-03T12:00:35+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/Gxad3-pmzqw/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-07-04",
  "duration_seconds": 1352,
  "transcript_sha256": "3fb495a8ba33de58709795ccb30e8f39ce4400a223a72c1cb05eaa2f3b8204c0",
  "transcript_text_chars": 20893,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.992292899408284,
    "text_chars": 20893,
    "completed_at": "2026-06-29 07:35:26"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:Gxad3-pmzqw
```

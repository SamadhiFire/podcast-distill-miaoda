...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/8s1vlXhm2t0/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/8s1vlXhm2t0/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/8s1vlXhm2t0/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:8s1vlXhm2t0",
  "platform": "youtube",
  "platform_id": "8s1vlXhm2t0",
  "source_id": "yt_bloomberg_originals",
  "source_name": "Bloomberg Originals",
  "category": "商业 / 财经 / 投资",
  "title": "Why China Is Building an AI Megaplex in the Desert",
  "url": "https://www.youtube.com/watch?v=8s1vlXhm2t0",
  "published_at": "2025-08-01T08:00:11+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/8s1vlXhm2t0/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-02",
  "duration_seconds": 722,
  "transcript_sha256": "60e806e0a578d352339f53e7e47c7e53c788f36d1814cc12ff370b93b3f98b0e",
  "transcript_text_chars": 10014,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9917728531855955,
    "text_chars": 10014,
    "completed_at": "2026-06-29 08:26:23"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:8s1vlXhm2t0
```

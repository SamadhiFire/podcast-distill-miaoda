...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/N5JDzS9MQYI/transcript.txt`
- 写入结果文件：`backfill/items/youtube/N5JDzS9MQYI/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:N5JDzS9MQYI",
  "platform": "youtube",
  "platform_id": "N5JDzS9MQYI",
...
把下面 JSON 写入 `backfill/items/youtube/N5JDzS9MQYI/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-02-13",
  "duration_seconds": 3753,
  "transcript_sha256": "00bfb5dba8661453f879435716f21bdf3b6932a6398c4aa7f6fd73694f19cca9",
  "transcript_text_chars": 62547,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9983799626965095,
    "text_chars": 62547,
    "completed_at": "2026-06-29 12:44:56"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:N5JDzS9MQYI
```

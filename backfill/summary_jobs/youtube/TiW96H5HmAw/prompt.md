...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/TiW96H5HmAw/transcript.txt`
- 写入结果文件：`backfill/items/youtube/TiW96H5HmAw/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:TiW96H5HmAw",
  "platform": "youtube",
  "platform_id": "TiW96H5HmAw",
...
把下面 JSON 写入 `backfill/items/youtube/TiW96H5HmAw/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-05-07",
  "duration_seconds": 2259,
  "transcript_sha256": "1839e2e5bcca1925c7ea949558dfe4037b4a424abc674ea972bd230793b84f78",
  "transcript_text_chars": 41017,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9993094289508633,
    "text_chars": 41017,
    "completed_at": "2026-06-29 13:41:29"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:TiW96H5HmAw
```

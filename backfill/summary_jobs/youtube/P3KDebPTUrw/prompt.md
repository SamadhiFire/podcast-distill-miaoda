...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/P3KDebPTUrw/transcript.txt`
- 写入结果文件：`backfill/items/youtube/P3KDebPTUrw/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:P3KDebPTUrw",
  "platform": "youtube",
  "platform_id": "P3KDebPTUrw",
  ...
  "published_at": "2026-06-28T12:30:05+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/P3KDebPTUrw/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-06-29",
  "duration_seconds": 4197,
  "transcript_sha256": "181495a88bcaa8e67ea8ece8d38e9eece7c763de6852b4df6a765b1a3f55b289",
  "transcript_text_chars": 72901,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9995708839647366,
    "text_chars": 72901,
    "completed_at": "2026-06-29 14:22:01"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:P3KDebPTUrw
```

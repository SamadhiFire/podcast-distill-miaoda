...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/lXUZvyajciY/transcript.txt`
- 写入结果文件：`backfill/items/youtube/lXUZvyajciY/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:lXUZvyajciY",
  "platform": "youtube",
  "platform_id": "lXUZvyajciY",
  ...
  "published_at": "2025-10-17T17:15:45+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/lXUZvyajciY/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-10-18",
  "duration_seconds": 8768,
  "transcript_sha256": "29b74c7b1d2001b982b2caba4efd1d97f634689ac04669ca53861abdb499e1ca",
  "transcript_text_chars": 175040,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9996167883211678,
    "text_chars": 175040,
    "completed_at": "2026-06-29 11:20:18"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:lXUZvyajciY
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/3ahfzR2aA-A/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/3ahfzR2aA-A/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/3ahfzR2aA-A/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:3ahfzR2aA-A",
  "platform": "youtube",
  "platform_id": "3ahfzR2aA-A",
  "source_id": "yt_bloomberg_china",
  "source_name": "Bloomberg The China Show",
  "category": "商业 / 财经 / 投资",
  "title": "Trump Imposes Sweeping Global Tariff Rates | Bloomberg: The China Show 08/01/2025",
  "url": "https://www.youtube.com/watch?v=3ahfzR2aA-A",
  "published_at": "2025-08-01T05:37:48+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/3ahfzR2aA-A/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-02",
  "duration_seconds": 5526,
  "transcript_sha256": "7f3a849e9ba63ec8784ffe9f980fb0455501bc24a98cdb7ec25df349c135a82d",
  "transcript_text_chars": 90255,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9989558450959102,
    "text_chars": 90255,
    "completed_at": "2026-06-29 08:25:51"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:3ahfzR2aA-A
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/7OLVwZeMCfY/transcript.txt`
- 写入结果文件：`backfill/items/youtube/7OLVwZeMCfY/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:7OLVwZeMCfY",
  "platform": "youtube",
  "platform_id": "7OLVwZeMCfY",
  ...
  "published_at": "2025-10-14T17:21:10+00:00",
  "report_date": "2025-10-15",
  "duration_seconds": 9727,
  "language": "en"
}
```
...
把下面 JSON 写入 `backfill/items/youtube/7OLVwZeMCfY/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-10-15",
  "duration_seconds": 9727,
  "transcript_sha256": "3de07116a316465d7b7e7ccdb061e8eb9d17e23e59756ebd9c0cdeec96974305",
  "transcript_text_chars": 163639,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9986552894006374,
    "text_chars": 163639,
    "completed_at": "2026-06-29 11:17:33"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:7OLVwZeMCfY
```

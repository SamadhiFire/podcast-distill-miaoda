...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/jiH7dBcBEcg/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/jiH7dBcBEcg/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/jiH7dBcBEcg/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:jiH7dBcBEcg",
  "platform": "youtube",
  "platform_id": "jiH7dBcBEcg",
  "source_id": "yt_interesting_times",
  "source_name": "Interesting Times with Ross Douthat",
  "category": "新闻 / 时评 / 全球议题",
  "title": "Abolish the Senate. End the Electoral College. Pack the Court. | Interesting Times with Ross Douthat",
  "url": "https://www.youtube.com/watch?v=jiH7dBcBEcg",
  "published_at": "2025-08-14T13:59:48+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/jiH7dBcBEcg/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-15",
  "duration_seconds": 3325,
  "transcript_sha256": "77ea276354470300d3df2560cb575f41b62fe29e561c5ed96cb97878ac664b5d",
  "transcript_text_chars": 56915,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9936932330827069,
    "text_chars": 56915,
    "completed_at": "2026-06-29 08:56:09"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:jiH7dBcBEcg
```

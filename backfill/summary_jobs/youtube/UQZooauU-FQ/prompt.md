...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/UQZooauU-FQ/transcript.txt`
- 写入结果文件：`backfill/items/youtube/UQZooauU-FQ/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:UQZooauU-FQ",
  "platform": "youtube",
  "platform_id": "UQZooauU-FQ",
  ...
  "url": "https://www.youtube.com/watch?v=UQZooauU-FQ",
  "published_at": "2026-04-29T08:00:37+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/UQZooauU-FQ/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-04-30",
  "duration_seconds": 1442,
  "transcript_sha256": "3995126a5214bde04275fc7afedfc1f48f99ddc94276d53995804847b1da1d27",
  "transcript_text_chars": 20481,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9911414701803052,
    "text_chars": 20481,
    "completed_at": "2026-06-29 13:34:20"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:UQZooauU-FQ
```

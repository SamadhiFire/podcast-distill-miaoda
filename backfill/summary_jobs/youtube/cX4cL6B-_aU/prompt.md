...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/cX4cL6B-_aU/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/cX4cL6B-_aU/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/cX4cL6B-_aU/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:cX4cL6B-_aU",
  "platform": "youtube",
  "platform_id": "cX4cL6B-_aU",
  ...
  "url": "https://www.youtube.com/watch?v=cX4cL6B-_aU",
  "published_at": "2025-08-17T11:00:56+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/cX4cL6B-_aU/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-18",
  "duration_seconds": 5352,
  "transcript_sha256": "ca796fb28f358b53ce68f6b929615bfb42575ddbd69d24f669e4d2fcb69afabb",
  "transcript_text_chars": 91341,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9990284005979073,
    "text_chars": 91341,
    "completed_at": "2026-06-29 09:01:30"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:cX4cL6B-_aU
```

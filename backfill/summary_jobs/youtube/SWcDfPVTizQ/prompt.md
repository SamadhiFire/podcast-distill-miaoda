...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/SWcDfPVTizQ/transcript.txt`
- 写入结果文件：`backfill/items/youtube/SWcDfPVTizQ/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:SWcDfPVTizQ",
  "platform": "youtube",
  "platform_id": "SWcDfPVTizQ",
  ...
  "published_at": "2025-10-19T11:00:43+00:00",
  "report_date": "2025-10-20",
  "duration_seconds": 4068,
  "language": "en"
}
```
...
把下面 JSON 写入 `backfill/items/youtube/SWcDfPVTizQ/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-10-20",
  "duration_seconds": 4068,
  "transcript_sha256": "f2523207cb929bfcfb7f3d0b41da6cd73d5eeb406ad2dd3aefa4f7ada0ced18e",
  "transcript_text_chars": 72150,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9988200589970501,
    "text_chars": 72150,
    "completed_at": "2026-06-29 11:20:57"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:SWcDfPVTizQ
```

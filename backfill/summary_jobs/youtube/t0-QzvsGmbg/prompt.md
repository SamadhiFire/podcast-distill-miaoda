...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/t0-QzvsGmbg/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/t0-QzvsGmbg/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/t0-QzvsGmbg/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:t0-QzvsGmbg",
  "platform": "youtube",
  "platform_id": "t0-QzvsGmbg",
  "source_id": "yt_the_daily",
  "source_name": "The Daily",
  "category": "新闻 / 时评 / 全球议题",
  "title": "The Most Closely Watched Trump Firing in Washington",
  "url": "https://www.youtube.com/watch?v=t0-QzvsGmbg",
  "published_at": "2025-08-05T14:25:18+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/t0-QzvsGmbg/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-06",
  "duration_seconds": 1762,
  "transcript_sha256": "c1eb644072277870250945a6153142e9d550b656c8a37e3fcf243ac8373cb844",
  "transcript_text_chars": 25086,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9985471055618615,
    "text_chars": 25086,
    "completed_at": "2026-06-29 09:52:21"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:t0-QzvsGmbg
```

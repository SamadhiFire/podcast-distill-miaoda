...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/L_7x6fEOYIU/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/L_7x6fEOYIU/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/L_7x6fEOYIU/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:L_7x6fEOYIU",
  "platform": "youtube",
  "platform_id": "L_7x6fEOYIU",
  "source_id": "yt_odd_lots_audio",
  "source_name": "Odd Lots Audio",
  "category": "商业 / 财经 / 投资",
  "title": "Lev Menand on Trump's Attempt to Fire the Fed's Lisa Cook | Odd Lots",
  "url": "https://www.youtube.com/watch?v=L_7x6fEOYIU",
  "published_at": "2025-08-27T12:57:16+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/L_7x6fEOYIU/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-28",
  "duration_seconds": 1722,
  "transcript_sha256": "5d1cf868d0d464979f2ff9dd3e85291a1abf86a10afb7a92aacda396af0f7398",
  "transcript_text_chars": 26489,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9950813008130082,
    "text_chars": 26489,
    "completed_at": "2026-06-29 09:35:47"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:L_7x6fEOYIU
```

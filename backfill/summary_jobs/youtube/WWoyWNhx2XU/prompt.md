...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/WWoyWNhx2XU/transcript.txt`
- 写入结果文件：`backfill/items/youtube/WWoyWNhx2XU/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:WWoyWNhx2XU",
  "platform": "youtube",
  "platform_id": "WWoyWNhx2XU",
  ...
  "published_at": "2025-07-20T11:00:52+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/WWoyWNhx2XU/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-07-21",
  "duration_seconds": 4499,
  "transcript_sha256": "ac48e5a82792ae8da5443bc662de37ec0c37690a7abfb548d137fd06509e80fa",
  "transcript_text_chars": 73106,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9989239831073571,
    "text_chars": 73106,
    "completed_at": "2026-06-29 08:08:42"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:WWoyWNhx2XU
```

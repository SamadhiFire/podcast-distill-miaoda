...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/NSFAIakPdmo/transcript.txt`
- 写入结果文件：`backfill/items/youtube/NSFAIakPdmo/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:NSFAIakPdmo",
  "platform": "youtube",
  "platform_id": "NSFAIakPdmo",
  ...
  "published_at": "2026-02-21T10:00:35+00:00",
  "report_date": "2026-02-22",
  "duration_seconds": 4181,
  "language": "en"
}
```
...
把下面 JSON 写入 `backfill/items/youtube/NSFAIakPdmo/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-02-22",
  "duration_seconds": 4181,
  "transcript_sha256": "ade80ad9d6b25e169accc0ce9af360bfb9a5db39b67c924b63532b692fc1c4eb",
  "transcript_text_chars": 64887,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9983829227457545,
    "text_chars": 64887,
    "completed_at": "2026-06-29 12:50:17"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:NSFAIakPdmo
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/by7Y9KoErTw/transcript.txt`
- 写入结果文件：`backfill/items/youtube/by7Y9KoErTw/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:by7Y9KoErTw",
  "platform": "youtube",
  "platform_id": "by7Y9KoErTw",
...
把下面 JSON 写入 `backfill/items/youtube/by7Y9KoErTw/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-05-29",
  "duration_seconds": 1499,
  "transcript_sha256": "3bb6aec7711af36a15c707a0b7beba24e2fa7cddcda2e3f9783f1bfba58541a6",
  "transcript_text_chars": 22093,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9962755170113409,
    "text_chars": 22093,
    "completed_at": "2026-06-29 13:59:39"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:by7Y9KoErTw
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/a06qSgfccZs/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/a06qSgfccZs/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/a06qSgfccZs/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:a06qSgfccZs",
  "platform": "youtube",
  "platform_id": "a06qSgfccZs",
...
把下面 JSON 写入 `backfill/items/youtube/a06qSgfccZs/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-27",
  "duration_seconds": 9316,
  "transcript_sha256": "38c772273cc30c38df6078ec66d9a19b5e99c7c5a2f66615de10e4bb8bfb1b83",
  "transcript_text_chars": 163973,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9995158866466294,
    "text_chars": 163973,
    "completed_at": "2026-06-29 09:34:12"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:a06qSgfccZs
```

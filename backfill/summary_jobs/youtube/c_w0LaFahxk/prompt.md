...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/c_w0LaFahxk/transcript.txt`
- 写入结果文件：`backfill/items/youtube/c_w0LaFahxk/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:c_w0LaFahxk",
  "platform": "youtube",
  "platform_id": "c_w0LaFahxk",
  ...
  "published_at": "2025-09-21T11:01:08+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/c_w0LaFahxk/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-09-22",
  "duration_seconds": 5784,
  "transcript_sha256": "5d0d90d91d71e2f80201d88ebb63445e9d1d3c0fb7b365ad29c43563de8caa9f",
  "transcript_text_chars": 103228,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9993084370677732,
    "text_chars": 103228,
    "completed_at": "2026-06-29 10:29:27"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:c_w0LaFahxk
```

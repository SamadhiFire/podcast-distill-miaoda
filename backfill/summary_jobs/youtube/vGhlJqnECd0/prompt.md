...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/vGhlJqnECd0/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/vGhlJqnECd0/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/vGhlJqnECd0/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:vGhlJqnECd0",
  "platform": "youtube",
  "platform_id": "vGhlJqnECd0",
  "source_id": "yt_no_priors",
  "source_name": "No Priors Podcast",
  "category": "科技 / AI / VC",
  "title": "No Priors Ep. 127 | With SemiAnalysis Founder and CEO Dylan Patel",
  "url": "https://www.youtube.com/watch?v=vGhlJqnECd0",
  "published_at": "2025-08-14T10:01:35+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/vGhlJqnECd0/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-15",
  "duration_seconds": 2838,
  "transcript_sha256": "e52226ee75e596e1fdc962d8eb57ff6ea36feb4ad762851f1a968036cf449abe",
  "transcript_text_chars": 57182,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9983932346723045,
    "text_chars": 57182,
    "completed_at": "2026-06-29 08:54:15"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:vGhlJqnECd0
```

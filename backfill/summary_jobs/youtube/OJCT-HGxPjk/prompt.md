...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/OJCT-HGxPjk/transcript.txt`
- 写入结果文件：`backfill/items/youtube/OJCT-HGxPjk/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:OJCT-HGxPjk",
  "platform": "youtube",
  "platform_id": "OJCT-HGxPjk",
  ...
  "published_at": "2026-01-06T13:00:59+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/OJCT-HGxPjk/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-01-07",
  "duration_seconds": 3698,
  "transcript_sha256": "63ce9214b8e559800edf0f41de5594266525441aacf580136090c0cd0af18df0",
  "transcript_text_chars": 65004,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9982752839372634,
    "text_chars": 65004,
    "completed_at": "2026-06-29 12:19:48"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:OJCT-HGxPjk
```

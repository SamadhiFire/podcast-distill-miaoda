...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/wS57SInZt8g/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/wS57SInZt8g/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/wS57SInZt8g/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:wS57SInZt8g",
  "platform": "youtube",
  "platform_id": "wS57SInZt8g",
  "source_id": "yt_bloomberg_originals",
  "source_name": "Bloomberg Originals",
  "category": "商业 / 财经 / 投资",
  "title": "Why Samsung Is Falling Behind in the AI Chips Race",
  "url": "https://www.youtube.com/watch?v=wS57SInZt8g",
  "published_at": "2025-08-08T08:01:14+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/wS57SInZt8g/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-09",
  "duration_seconds": 630,
  "transcript_sha256": "d81185c5c533d83a612479e2d1968e59390d5ccf56308ef6b70bd9936dd9d7c4",
  "transcript_text_chars": 9417,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9814761904761905,
    "text_chars": 9417,
    "completed_at": "2026-06-29 08:42:26"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:wS57SInZt8g
```

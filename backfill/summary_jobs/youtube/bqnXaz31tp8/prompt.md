...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/bqnXaz31tp8/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/bqnXaz31tp8/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/bqnXaz31tp8/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:bqnXaz31tp8",
  "platform": "youtube",
  "platform_id": "bqnXaz31tp8",
  "source_id": "yt_bloomberg_originals",
  "source_name": "Bloomberg Originals",
  "category": "商业 / 财经 / 投资",
  "title": "Why YouTube Still Rules the Internet",
  "url": "https://www.youtube.com/watch?v=bqnXaz31tp8",
  "published_at": "2025-08-29T08:00:50+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/bqnXaz31tp8/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-30",
  "duration_seconds": 845,
  "transcript_sha256": "40459ed193dae2f716fb88d82eb24e7b8412c2b1518e9a08151e366669ac1efb",
  "transcript_text_chars": 11223,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.974473372781065,
    "text_chars": 11223,
    "completed_at": "2026-06-29 09:41:11"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:bqnXaz31tp8
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/R5_ypwiRIyo/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/R5_ypwiRIyo/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/R5_ypwiRIyo/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:R5_ypwiRIyo",
  "platform": "youtube",
  "platform_id": "R5_ypwiRIyo",
  ...
  "published_at": "2025-08-03T11:00:41+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/R5_ypwiRIyo/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-04",
  "duration_seconds": 4777,
  "transcript_sha256": "799f1883a1aa0ce543dfe0b0a6d82158e349a54873434a902ca00f835389d490",
  "transcript_text_chars": 76422,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9988528365082688,
    "text_chars": 76422,
    "completed_at": "2026-06-29 08:27:38"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:R5_ypwiRIyo
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/-HzgcbRXUK8/transcript.txt`
- 写入结果文件：`backfill/items/youtube/-HzgcbRXUK8/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:-HzgcbRXUK8",
  "platform": "youtube",
  "platform_id": "-HzgcbRXUK8",
  ...
  "published_at": "2025-07-23T18:39:15+00:00",
  "report_date": "2025-07-24",
  "duration_seconds": 8895,
  "language": "en"
}
```
...
把下面 JSON 写入 `backfill/items/youtube/-HzgcbRXUK8/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-07-24",
  "duration_seconds": 8895,
  "transcript_sha256": "ab8bd9a405a2cffc3a9b8072d5cb7eb832546800d32478a86d7ac6fd77bf3e83",
  "transcript_text_chars": 147519,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9987992130410343,
    "text_chars": 147519,
    "completed_at": "2026-06-29 08:13:49"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:-HzgcbRXUK8
```

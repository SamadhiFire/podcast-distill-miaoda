...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/9kTMdaC0Fz0/transcript.txt`
- 写入结果文件：`backfill/items/youtube/9kTMdaC0Fz0/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:9kTMdaC0Fz0",
  "platform": "youtube",
  "platform_id": "9kTMdaC0Fz0",
  ...
  "published_at": "2026-03-26T17:15:37+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/9kTMdaC0Fz0/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-03-27",
  "duration_seconds": 3580,
  "transcript_sha256": "93a431db7944ab3daa024c30bf2d7fd0cf5721e00cabcf927b633ca3a4c8ffa6",
  "transcript_text_chars": 65904,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9970837988826815,
    "text_chars": 65904,
    "completed_at": "2026-06-29 13:13:06"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:9kTMdaC0Fz0
```

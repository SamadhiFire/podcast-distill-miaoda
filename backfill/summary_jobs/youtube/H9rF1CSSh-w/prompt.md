...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/H9rF1CSSh-w/transcript.txt`
- 写入结果文件：`backfill/items/youtube/H9rF1CSSh-w/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:H9rF1CSSh-w",
  "platform": "youtube",
  "platform_id": "H9rF1CSSh-w",
  ...
  "published_at": "2026-03-11T20:21:39+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/H9rF1CSSh-w/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-03-12",
  "duration_seconds": 18612,
  "transcript_sha256": "eec898ae88312b426d0bface455c089aa4093d7ea820e09da1231772a0444e5f",
  "transcript_text_chars": 245140,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9992356544165055,
    "text_chars": 245140,
    "completed_at": "2026-06-29 13:02:01"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:H9rF1CSSh-w
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/xsQkwbQSMfQ/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/xsQkwbQSMfQ/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/xsQkwbQSMfQ/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:xsQkwbQSMfQ",
  "platform": "youtube",
  "platform_id": "xsQkwbQSMfQ",
  "source_id": "yt_bloomberg_surveillance",
  "source_name": "Bloomberg Surveillance Full Shows",
  "category": "商业 / 财经 / 投资",
  "title": "Bloomberg Surveillance 8/14/2025",
  "url": "https://www.youtube.com/watch?v=xsQkwbQSMfQ",
  "published_at": "2025-08-14T15:35:39+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/xsQkwbQSMfQ/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-15",
  "duration_seconds": 8653,
  "transcript_sha256": "5365672f11d55e479d57621255655a04abcc072329912afd8e67b352d3843799",
  "transcript_text_chars": 153299,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.999736507569629,
    "text_chars": 153299,
    "completed_at": "2026-06-29 08:56:38"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:xsQkwbQSMfQ
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/vtugjs2chdA/transcript.txt`
- 写入结果文件：`backfill/items/youtube/vtugjs2chdA/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:vtugjs2chdA",
  "platform": "youtube",
  "platform_id": "vtugjs2chdA",
  ...
  "published_at": "2026-01-21T13:01:56+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/vtugjs2chdA/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-01-22",
  "duration_seconds": 2388,
  "transcript_sha256": "2a19b411b442dd81488644762747d773a9d2d5ec2fdfcac9fe532e276e45d91c",
  "transcript_text_chars": 46212,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9859966499162479,
    "text_chars": 46212,
    "completed_at": "2026-06-29 12:28:46"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:vtugjs2chdA
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/fTqINzeudJ4/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/fTqINzeudJ4/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/fTqINzeudJ4/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:fTqINzeudJ4",
  "platform": "youtube",
  "platform_id": "fTqINzeudJ4",
  "source_id": "yt_bg2pod",
  "source_name": "Bg2 Pod",
  "category": "科技 / AI / VC",
  "title": "China Open-Source, Compute Arms Race, Reordering Global Trade | BG2 w/ Bill Gurley and Brad Gerstner",
  "url": "https://www.youtube.com/watch?v=fTqINzeudJ4",
  "published_at": "2025-07-31T15:25:00+00:00",
  "report_date": "2025-08-01",
  "duration_seconds": 3861,
  "language": "en"
}
```
...
把下面 JSON 写入 `backfill/items/youtube/fTqINzeudJ4/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-01",
  "duration_seconds": 3861,
  "transcript_sha256": "5386ec3306d45c5022198cd1cc28a2d037818aa08906059d35c030aedd39f413",
  "transcript_text_chars": 61348,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9994299404299404,
    "text_chars": 61348,
    "completed_at": "2026-06-29 08:25:00"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:fTqINzeudJ4
```

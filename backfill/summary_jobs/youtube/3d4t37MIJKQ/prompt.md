...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/3d4t37MIJKQ/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/3d4t37MIJKQ/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/3d4t37MIJKQ/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:3d4t37MIJKQ",
  "platform": "youtube",
  "platform_id": "3d4t37MIJKQ",
  "source_id": "yt_bloomberg_china",
  "source_name": "Bloomberg The China Show",
  "category": "商业 / 财经 / 投资",
  "title": "Nvidia Reportedly Seeking Halt to H20 Production | The China Show 8/22/2025",
  "url": "https://www.youtube.com/watch?v=3d4t37MIJKQ",
  "published_at": "2025-08-22T05:50:35+00:00",
  "report_date": "2025-08-23",
  "duration_seconds": 5475,
  "language": "en"
}
```
...
把下面 JSON 写入 `backfill/items/youtube/3d4t37MIJKQ/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-23",
  "duration_seconds": 5475,
  "transcript_sha256": "5d8382b6c8b43d5f1a073fb4a4525ed2f95d67a1bc4a240c8a5d630d4ca074e9",
  "transcript_text_chars": 91656,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.999744292237443,
    "text_chars": 91656,
    "completed_at": "2026-06-29 09:24:48"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:3d4t37MIJKQ
```

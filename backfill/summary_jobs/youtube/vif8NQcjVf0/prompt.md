...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/vif8NQcjVf0/transcript.txt`
- 写入结果文件：`backfill/items/youtube/vif8NQcjVf0/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:vif8NQcjVf0",
  "platform": "youtube",
  "platform_id": "vif8NQcjVf0",
  ...
  "published_at": "2026-03-23T16:24:14+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/vif8NQcjVf0/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-03-24",
  "duration_seconds": 8759,
  "transcript_sha256": "2240fc03524801cea00e3e02266453285c6ed8ccc357a3488ad2a3c32e1d07de",
  "transcript_text_chars": 128992,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9993651101723942,
    "text_chars": 128992,
    "completed_at": "2026-06-29 13:10:28"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:vif8NQcjVf0
```

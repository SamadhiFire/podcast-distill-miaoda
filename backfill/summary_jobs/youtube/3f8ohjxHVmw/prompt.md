...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/3f8ohjxHVmw/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/3f8ohjxHVmw/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/3f8ohjxHVmw/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:3f8ohjxHVmw",
  "platform": "youtube",
  "platform_id": "3f8ohjxHVmw",
  ...
  "url": "https://www.youtube.com/watch?v=3f8ohjxHVmw",
  "published_at": "2025-08-01T06:00:01+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/3f8ohjxHVmw/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-02",
  "duration_seconds": 907,
  "transcript_sha256": "88616a2d8157eb08961199ceace0ce60fffca40b1474d9f906e35131b0e23b2d",
  "transcript_text_chars": 11597,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9942116868798236,
    "text_chars": 11597,
    "completed_at": "2026-06-29 08:26:05"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:3f8ohjxHVmw
```

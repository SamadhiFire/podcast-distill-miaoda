...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/8gixdXd3UK8/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/8gixdXd3UK8/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/8gixdXd3UK8/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:8gixdXd3UK8",
  "platform": "youtube",
  "platform_id": "8gixdXd3UK8",
  "source_id": "yt_bloomberg_china",
  "source_name": "Bloomberg The China Show",
  "category": "商业 / 财经 / 投资",
  "title": "China, HK Activity Gauges Show Signs of Improvement | The China Show 8/5/2025",
  "url": "https://www.youtube.com/watch?v=8gixdXd3UK8",
  "published_at": "2025-08-05T06:18:04+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/8gixdXd3UK8/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-06",
  "duration_seconds": 5543,
  "transcript_sha256": "e884ff0c5a49b4097fc23a1ec71ed07447ab0e85fbca413e986e3dee5d2832a9",
  "transcript_text_chars": 86799,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9988959047447231,
    "text_chars": 86799,
    "completed_at": "2026-06-30 02:10:41"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:8gixdXd3UK8
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/3cDHx2_QbPE/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/3cDHx2_QbPE/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/3cDHx2_QbPE/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:3cDHx2_QbPE",
  "platform": "youtube",
  "platform_id": "3cDHx2_QbPE",
  ...
  "published_at": "2025-08-15T15:26:07+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/3cDHx2_QbPE/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-16",
  "duration_seconds": 4102,
  "transcript_sha256": "d16a2d5962a27515537cd22571329be5bdbf0eede90d2096cb8d1c27a1502d8c",
  "transcript_text_chars": 82566,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9993171623598244,
    "text_chars": 82566,
    "completed_at": "2026-06-29 08:58:45"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:3cDHx2_QbPE
```

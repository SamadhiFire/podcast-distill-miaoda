...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/yUohoaC8_Hs/transcript.txt`
- 写入结果文件：`backfill/items/youtube/yUohoaC8_Hs/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:yUohoaC8_Hs",
  "platform": "youtube",
  "platform_id": "yUohoaC8_Hs",
  ...
  "published_at": "2026-04-19T12:31:14+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/yUohoaC8_Hs/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-04-20",
  "duration_seconds": 5711,
  "transcript_sha256": "1802bbf35ed33ebabb13cce76b9bb0b997d3e78cbfcecec8299e3f9dd49886df",
  "transcript_text_chars": 92092,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9993696375415864,
    "text_chars": 92092,
    "completed_at": "2026-06-29 13:28:03"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:yUohoaC8_Hs
```

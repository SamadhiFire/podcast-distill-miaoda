...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/TOsNrV3bXtQ/transcript.txt`
- 写入结果文件：`backfill/items/youtube/TOsNrV3bXtQ/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:TOsNrV3bXtQ",
  "platform": "youtube",
  "platform_id": "TOsNrV3bXtQ",
  ...
  "published_at": "2025-12-19T21:13:15+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/TOsNrV3bXtQ/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-12-20",
  "duration_seconds": 2447,
  "transcript_sha256": "dc6b44d98e5c1953ebaa107c7f08bfd712bfbeed330ad12c27423b5c54927456",
  "transcript_text_chars": 44054,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9979566816510013,
    "text_chars": 44054,
    "completed_at": "2026-06-29 12:12:19"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:TOsNrV3bXtQ
```

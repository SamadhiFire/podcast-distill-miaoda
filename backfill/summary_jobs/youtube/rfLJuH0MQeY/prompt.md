...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/rfLJuH0MQeY/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/rfLJuH0MQeY/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/rfLJuH0MQeY/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:rfLJuH0MQeY",
  "platform": "youtube",
  "platform_id": "rfLJuH0MQeY",
  "source_id": "yt_bloomberg_surveillance",
  "source_name": "Bloomberg Surveillance Full Shows",
  "category": "商业 / 财经 / 投资",
  "title": "Bloomberg Surveillance 8/20/2025",
  "url": "https://www.youtube.com/watch?v=rfLJuH0MQeY",
  "published_at": "2025-08-20T16:15:16+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/rfLJuH0MQeY/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-21",
  "duration_seconds": 8656,
  "transcript_sha256": "560e71e9ef39e822594a4d12de98261ab7e5898d68b072f4318911bfcdcce94f",
  "transcript_text_chars": 152020,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9995563770794824,
    "text_chars": 152020,
    "completed_at": "2026-06-29 09:21:35"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:rfLJuH0MQeY
```

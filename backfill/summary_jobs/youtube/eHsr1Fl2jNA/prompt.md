...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/eHsr1Fl2jNA/transcript.txt`
- 写入结果文件：`backfill/items/youtube/eHsr1Fl2jNA/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:eHsr1Fl2jNA",
  "platform": "youtube",
  "platform_id": "eHsr1Fl2jNA",
  ...
  "published_at": "2026-05-06T16:50:12+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/eHsr1Fl2jNA/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-05-07",
  "duration_seconds": 542,
  "transcript_sha256": "2ab02b16bb6c3a445240728459623597f709b05ce185878724dcd3bcfe5bf93d",
  "transcript_text_chars": 6898,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.990970479704797,
    "text_chars": 6898,
    "completed_at": "2026-06-29 13:42:41"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:eHsr1Fl2jNA
```

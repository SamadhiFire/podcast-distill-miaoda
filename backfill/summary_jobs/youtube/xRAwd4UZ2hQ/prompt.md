...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/xRAwd4UZ2hQ/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/xRAwd4UZ2hQ/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/xRAwd4UZ2hQ/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:xRAwd4UZ2hQ",
  "platform": "youtube",
  "platform_id": "xRAwd4UZ2hQ",
  "source_id": "yt_odd_lots_audio",
  "source_name": "Odd Lots Audio",
  "category": "商业 / 财经 / 投资",
  "title": "Adam Posen on a Surreal Jackson Hole in a Post-American World | Odd Lots",
  "url": "https://www.youtube.com/watch?v=xRAwd4UZ2hQ",
  "published_at": "2025-08-26T14:41:59+00:00",
  "report_date": "2025-08-27",
  "duration_seconds": 3462,
  "language": "en"
}
```
...
把下面 JSON 写入 `backfill/items/youtube/xRAwd4UZ2hQ/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-27",
  "duration_seconds": 3462,
  "transcript_sha256": "7ca702e596eda1ab7adf8eaf06228f1af05fecc8666132b57ec12a07e1570a3a",
  "transcript_text_chars": 55070,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9987694974003466,
    "text_chars": 55070,
    "completed_at": "2026-06-29 09:34:37"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:xRAwd4UZ2hQ
```

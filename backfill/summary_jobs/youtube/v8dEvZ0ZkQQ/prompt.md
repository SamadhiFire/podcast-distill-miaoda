...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/v8dEvZ0ZkQQ/transcript.txt`
- 写入结果文件：`backfill/items/youtube/v8dEvZ0ZkQQ/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:v8dEvZ0ZkQQ",
  "platform": "youtube",
  "platform_id": "v8dEvZ0ZkQQ",
  "source_id": "yt_the_daily",
  "source_name": "The Daily",
  "category": "新闻 / 时评 / 全球议题",
  "title": "Can Trump Force Blue Cities to Cooperate With ICE?",
  "url": "https://www.youtube.com/watch?v=v8dEvZ0ZkQQ",
  "published_at": "2026-02-02T15:23:02+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/v8dEvZ0ZkQQ/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-02-03",
  "duration_seconds": 1954,
  "transcript_sha256": "aa99d547c044892ce60aeb00f18bcf25a1f5f697cbfef4530231d32189324ce5",
  "transcript_text_chars": 27154,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.996806550665302,
    "text_chars": 27154,
    "completed_at": "2026-06-29 12:37:14"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:v8dEvZ0ZkQQ
```

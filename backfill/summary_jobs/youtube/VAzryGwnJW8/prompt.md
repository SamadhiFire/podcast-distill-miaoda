...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/VAzryGwnJW8/transcript.txt`
- 写入结果文件：`backfill/items/youtube/VAzryGwnJW8/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:VAzryGwnJW8",
  "platform": "youtube",
  "platform_id": "VAzryGwnJW8",
...
把下面 JSON 写入 `backfill/items/youtube/VAzryGwnJW8/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-03-17",
  "duration_seconds": 1855,
  "transcript_sha256": "256871ee044909510f10f567ca234fccf0da672b97843aa106e28880f34158a0",
  "transcript_text_chars": 30822,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9990296495956874,
    "text_chars": 30822,
    "completed_at": "2026-06-29 13:05:52"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:VAzryGwnJW8
```

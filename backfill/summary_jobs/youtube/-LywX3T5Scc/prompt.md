...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/-LywX3T5Scc/transcript.txt`
- 写入结果文件：`backfill/items/youtube/-LywX3T5Scc/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:-LywX3T5Scc",
  "platform": "youtube",
  "platform_id": "-LywX3T5Scc",
  ...
  "published_at": "2025-11-02T13:30:00+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/-LywX3T5Scc/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-11-03",
  "duration_seconds": 3970,
  "transcript_sha256": "f71fd17199bd0bc1b5d7cd00195db05ae3787f3a26523e8d6b167ff70ffea434",
  "transcript_text_chars": 73190,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9987909319899244,
    "text_chars": 73190,
    "completed_at": "2026-06-29 11:32:10"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:-LywX3T5Scc
```

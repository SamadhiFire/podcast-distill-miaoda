...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/KkddA1vApuM/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/KkddA1vApuM/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/KkddA1vApuM/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:KkddA1vApuM",
  "platform": "youtube",
  "platform_id": "KkddA1vApuM",
  "source_id": "yt_bloomberg_surveillance",
  "source_name": "Bloomberg Surveillance Full Shows",
  "category": "商业 / 财经 / 投资",
  "title": "Bloomberg Surveillance 8/12/2025",
  "url": "https://www.youtube.com/watch?v=KkddA1vApuM",
  "published_at": "2025-08-12T16:10:54+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/KkddA1vApuM/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-13",
  "duration_seconds": 8656,
  "transcript_sha256": "5fc93ffcd8ea754c13d12f094b8fc338965a47539c197aea4dc02ae8c7931472",
  "transcript_text_chars": 155330,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9996487985212569,
    "text_chars": 155330,
    "completed_at": "2026-06-29 08:49:41"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:KkddA1vApuM
```

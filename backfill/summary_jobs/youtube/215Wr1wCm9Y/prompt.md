...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/215Wr1wCm9Y/transcript.txt`
- 写入结果文件：`backfill/items/youtube/215Wr1wCm9Y/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:215Wr1wCm9Y",
  "platform": "youtube",
  "platform_id": "215Wr1wCm9Y",
  ...
  "published_at": "2025-10-13T22:00:04+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/215Wr1wCm9Y/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-10-15",
  "duration_seconds": 2853,
  "transcript_sha256": "b05bd3886e317d4af0060fec34f06d0bbbdd1495dcfc02081d237ff1cda627ca",
  "transcript_text_chars": 45363,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.994105152471083,
    "text_chars": 45363,
    "completed_at": "2026-06-29 11:16:37"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:215Wr1wCm9Y
```

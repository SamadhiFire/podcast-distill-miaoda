...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/dduQeaqmpnI/transcript.txt`
- 写入结果文件：`backfill/items/youtube/dduQeaqmpnI/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:dduQeaqmpnI",
  "platform": "youtube",
  "platform_id": "dduQeaqmpnI",
  ...
  "published_at": "2025-12-07T13:30:48+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/dduQeaqmpnI/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-12-08",
  "duration_seconds": 4232,
  "transcript_sha256": "a5717df4b4334143719fe495b80bc6f0915d3213af1b3f4b1c9d51dc0103f82c",
  "transcript_text_chars": 78622,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9987521266540643,
    "text_chars": 78622,
    "completed_at": "2026-06-29 12:00:56"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:dduQeaqmpnI
```

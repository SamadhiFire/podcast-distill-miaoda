...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/XCLODgdCmKA/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/XCLODgdCmKA/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/XCLODgdCmKA/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:XCLODgdCmKA",
  "platform": "youtube",
  "platform_id": "XCLODgdCmKA",
  ...
  "published_at": "2025-08-21T16:55:24+00:00",
  "report_date": "2025-08-22",
  "duration_seconds": 6321,
  "language": "en"
}
```
...
把下面 JSON 写入 `backfill/items/youtube/XCLODgdCmKA/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-22",
  "duration_seconds": 6321,
  "transcript_sha256": "6cec27571d2199e1cebbfcc8aaf71d2b24b37d7d7682d0f861c1de45ba6029f0",
  "transcript_text_chars": 127994,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9993355481727575,
    "text_chars": 127994,
    "completed_at": "2026-06-29 09:24:27"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:XCLODgdCmKA
```

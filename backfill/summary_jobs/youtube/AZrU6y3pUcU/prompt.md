...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/AZrU6y3pUcU/transcript.txt`
- 写入结果文件：`backfill/items/youtube/AZrU6y3pUcU/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:AZrU6y3pUcU",
  "platform": "youtube",
  "platform_id": "AZrU6y3pUcU",
  ...
  "published_at": "2026-06-26T10:25:40+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/AZrU6y3pUcU/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-06-27",
  "duration_seconds": 2179,
  "transcript_sha256": "69d870a5c4423f53f688fad66cf593410bd7341a56ddd61b2b071c96897b207e",
  "transcript_text_chars": 40260,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9979256539697109,
    "text_chars": 40260,
    "completed_at": "2026-06-29 14:21:16"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:AZrU6y3pUcU
```

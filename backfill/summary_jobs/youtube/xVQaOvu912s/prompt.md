...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/xVQaOvu912s/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/xVQaOvu912s/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/xVQaOvu912s/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:xVQaOvu912s",
  "platform": "youtube",
  "platform_id": "xVQaOvu912s",
  "source_id": "yt_odd_lots_audio",
  "source_name": "Odd Lots Audio",
  "category": "商业 / 财经 / 投资",
  "title": "Lots More With Skanda Amarnath on This Moment in Macro | Odd Lots",
  "url": "https://www.youtube.com/watch?v=xVQaOvu912s",
  "published_at": "2025-08-15T13:48:00+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/xVQaOvu912s/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-16",
  "duration_seconds": 1756,
  "transcript_sha256": "35bb1693a487a7def20390bf31bb93cd50f0f325a4e24aed2e1ff1c7b84d1061",
  "transcript_text_chars": 32463,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9977676537585422,
    "text_chars": 32463,
    "completed_at": "2026-06-29 08:58:11"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:xVQaOvu912s
```

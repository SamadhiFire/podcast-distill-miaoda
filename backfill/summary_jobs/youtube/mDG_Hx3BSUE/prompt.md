...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/mDG_Hx3BSUE/transcript.txt`
- 写入结果文件：`backfill/items/youtube/mDG_Hx3BSUE/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:mDG_Hx3BSUE",
  "platform": "youtube",
  "platform_id": "mDG_Hx3BSUE",
  ...
  "published_at": "2026-03-13T16:26:57+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/mDG_Hx3BSUE/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-03-14",
  "duration_seconds": 9045,
  "transcript_sha256": "8c9599c8a8ce9e47e86f41eca4f951efa5865ba61857b049faf8000336dbe608",
  "transcript_text_chars": 165176,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9997124378109452,
    "text_chars": 165176,
    "completed_at": "2026-06-29 13:04:13"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:mDG_Hx3BSUE
```

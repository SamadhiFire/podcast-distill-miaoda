...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/KtKJ3A6DWTs/transcript.txt`
- 写入结果文件：`backfill/items/youtube/KtKJ3A6DWTs/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:KtKJ3A6DWTs",
  "platform": "youtube",
  "platform_id": "KtKJ3A6DWTs",
  ...
  "published_at": "2026-01-15T13:30:21+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/KtKJ3A6DWTs/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-01-16",
  "duration_seconds": 5196,
  "transcript_sha256": "16bd0a3dd0206a719002076c1456e34aa536aa8be3f8c9e54e5eaee33776da8f",
  "transcript_text_chars": 101371,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9989376443418013,
    "text_chars": 101371,
    "completed_at": "2026-06-29 12:25:57"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:KtKJ3A6DWTs
```

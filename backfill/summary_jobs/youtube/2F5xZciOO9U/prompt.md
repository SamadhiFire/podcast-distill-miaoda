...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/2F5xZciOO9U/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/2F5xZciOO9U/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/2F5xZciOO9U/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:2F5xZciOO9U",
  "platform": "youtube",
  "platform_id": "2F5xZciOO9U",
  "source_id": "yt_odd_lots_audio",
  "source_name": "Odd Lots Audio",
  "category": "商业 / 财经 / 投资",
  "title": "Why Austan Goolsbee Is Still Concerned About Inflation | Odd Lots",
  "url": "https://www.youtube.com/watch?v=2F5xZciOO9U",
  "published_at": "2025-08-25T15:30:00+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/2F5xZciOO9U/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-26",
  "duration_seconds": 2075,
  "transcript_sha256": "068a0bf04556d7305acee077d1dad383f64545f0876cd0705c2ce72ee8224b6c",
  "transcript_text_chars": 29933,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9981489156626506,
    "text_chars": 29933,
    "completed_at": "2026-06-29 09:30:48"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:2F5xZciOO9U
```

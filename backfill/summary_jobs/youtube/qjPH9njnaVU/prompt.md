...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/qjPH9njnaVU/transcript.txt`
- 写入结果文件：`backfill/items/youtube/qjPH9njnaVU/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:qjPH9njnaVU",
  "platform": "youtube",
  "platform_id": "qjPH9njnaVU",
  ...
  "published_at": "2025-09-30T19:27:11+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/qjPH9njnaVU/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-10-01",
  "duration_seconds": 16450,
  "transcript_sha256": "f0d4e18425a64f8d7e615b054e959826bf9171cf83e70bb49240b718062c6ccb",
  "transcript_text_chars": 190098,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9992559270516717,
    "text_chars": 190098,
    "completed_at": "2026-06-29 11:07:35"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:qjPH9njnaVU
```

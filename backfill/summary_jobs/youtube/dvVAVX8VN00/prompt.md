...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/dvVAVX8VN00/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/dvVAVX8VN00/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/dvVAVX8VN00/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:dvVAVX8VN00",
  "platform": "youtube",
  "platform_id": "dvVAVX8VN00",
  "source_id": "yt_bloomberg_surveillance",
  "source_name": "Bloomberg Surveillance Full Shows",
  "category": "商业 / 财经 / 投资",
  "title": "Bloomberg Surveillance 8/29/2025",
  "url": "https://www.youtube.com/watch?v=dvVAVX8VN00",
  "published_at": "2025-08-29T15:57:30+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/dvVAVX8VN00/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-30",
  "duration_seconds": 8652,
  "transcript_sha256": "5ef840c929647ac9d95c4d6628819f457ab49c3dc0871fdcdd003da6ed6d9aff",
  "transcript_text_chars": 147394,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9993526352288488,
    "text_chars": 147394,
    "completed_at": "2026-06-29 09:42:15"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:dvVAVX8VN00
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/V2JNosW9o8M/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/V2JNosW9o8M/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/V2JNosW9o8M/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:V2JNosW9o8M",
  "platform": "youtube",
  "platform_id": "V2JNosW9o8M",
  "source_id": "yt_odd_lots_audio",
  "source_name": "Odd Lots Audio",
  "category": "商业 / 财经 / 投资",
  "title": "Lots More on What Just Happened With the Fed at Jackson Hole | Odd Lots",
  "url": "https://www.youtube.com/watch?v=V2JNosW9o8M",
  "published_at": "2025-08-22T21:57:35+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/V2JNosW9o8M/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-23",
  "duration_seconds": 1081,
  "transcript_sha256": "17b53fbdf54a9396bd0e9515f5a4bca70a48725a060198f07bad5c25c6427727",
  "transcript_text_chars": 18854,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9952997224791861,
    "text_chars": 18854,
    "completed_at": "2026-06-29 09:26:30"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:V2JNosW9o8M
```

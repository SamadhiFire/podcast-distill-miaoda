...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/5-MhvLoueiE/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/5-MhvLoueiE/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/5-MhvLoueiE/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:5-MhvLoueiE",
  "platform": "youtube",
  "platform_id": "5-MhvLoueiE",
  "source_id": "yt_the_daily",
  "source_name": "The Daily",
  "category": "新闻 / 时评 / 全球议题",
  "title": "Republican Town Halls Turned Ugly. One Congressman Kept Doing Them Anyway.",
  "url": "https://www.youtube.com/watch?v=5-MhvLoueiE",
  "published_at": "2025-08-18T14:28:38+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/5-MhvLoueiE/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-19",
  "duration_seconds": 2383,
  "transcript_sha256": "365c9b6586757bf756014835a3380f11f2aaf797d6e46302514517aefa1a7e0c",
  "transcript_text_chars": 34575,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9964246747796894,
    "text_chars": 34575,
    "completed_at": "2026-06-29 09:08:10"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:5-MhvLoueiE
```

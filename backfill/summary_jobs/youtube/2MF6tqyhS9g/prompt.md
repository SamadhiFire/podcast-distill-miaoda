...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/2MF6tqyhS9g/transcript.txt`
- 写入结果文件：`backfill/items/youtube/2MF6tqyhS9g/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
把下面 JSON 写入 `backfill/items/youtube/2MF6tqyhS9g/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "transcript_text_chars": 35937,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9946169772256729,
    "text_chars": 35937,
    "completed_at": "2026-06-29 14:00:24"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:2MF6tqyhS9g
```

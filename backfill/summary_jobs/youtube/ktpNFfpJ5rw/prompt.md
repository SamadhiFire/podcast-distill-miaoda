...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/ktpNFfpJ5rw/transcript.txt`
- 写入结果文件：`backfill/items/youtube/ktpNFfpJ5rw/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:ktpNFfpJ5rw",
  "platform": "youtube",
  "platform_id": "ktpNFfpJ5rw",
  ...
  "url": "https://www.youtube.com/watch?v=ktpNFfpJ5rw",
  "published_at": "2026-02-20T13:00:47+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/ktpNFfpJ5rw/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-02-21",
  "duration_seconds": 1418,
  "transcript_sha256": "edcce129973a449fa49736432ff1c896a49f8311055be509f1a01433335573c7",
  "transcript_text_chars": 24857,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9977425952045134,
    "text_chars": 24857,
    "completed_at": "2026-06-29 12:49:52"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:ktpNFfpJ5rw
```

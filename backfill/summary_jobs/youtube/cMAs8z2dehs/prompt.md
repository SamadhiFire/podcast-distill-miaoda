...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/cMAs8z2dehs/transcript.txt`
- 写入结果文件：`backfill/items/youtube/cMAs8z2dehs/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:cMAs8z2dehs",
  "platform": "youtube",
  "platform_id": "cMAs8z2dehs",
  ...
  "published_at": "2026-06-11T12:00:05+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/cMAs8z2dehs/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-06-12",
  "duration_seconds": 3069,
  "transcript_sha256": "e7b0592e39445e283a6db3748edbf7e87c277710527072862a9e5eeca6affd8d",
  "transcript_text_chars": 56925,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9979846855653307,
    "text_chars": 56925,
    "completed_at": "2026-06-29 14:09:20"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:cMAs8z2dehs
```

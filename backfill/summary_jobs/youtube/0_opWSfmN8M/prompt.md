...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/0_opWSfmN8M/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/0_opWSfmN8M/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/0_opWSfmN8M/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:0_opWSfmN8M",
  "platform": "youtube",
  "platform_id": "0_opWSfmN8M",
  ...
  "published_at": "2025-08-21T11:00:01+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/0_opWSfmN8M/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-22",
  "duration_seconds": 5000,
  "transcript_sha256": "891fff07edad43ef06dc1ecd6c4547a459adcc336ca760ad03593cae102b59b8",
  "transcript_text_chars": 74528,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.999008,
    "text_chars": 74528,
    "completed_at": "2026-06-29 09:22:33"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:0_opWSfmN8M
```

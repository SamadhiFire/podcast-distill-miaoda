...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/I_0Kuf6Aa2c/transcript.txt`
- 写入结果文件：`backfill/items/youtube/I_0Kuf6Aa2c/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:I_0Kuf6Aa2c",
  "platform": "youtube",
  "platform_id": "I_0Kuf6Aa2c",
  ...
  "published_at": "2026-05-01T15:42:26+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/I_0Kuf6Aa2c/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-05-02",
  "duration_seconds": 1636,
  "transcript_sha256": "cce7ffd267e7375a236f28ce834688cdd8b1c894acbd6f8c69deca96d3db8943",
  "transcript_text_chars": 25283,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9960146699266503,
    "text_chars": 25283,
    "completed_at": "2026-06-29 13:38:03"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:I_0Kuf6Aa2c
```

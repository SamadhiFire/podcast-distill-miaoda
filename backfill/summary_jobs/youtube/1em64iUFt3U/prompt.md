...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/1em64iUFt3U/transcript.txt`
- 写入结果文件：`backfill/items/youtube/1em64iUFt3U/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:1em64iUFt3U",
  "platform": "youtube",
  "platform_id": "1em64iUFt3U",
  ...
  "published_at": "2026-01-18T13:30:02+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/1em64iUFt3U/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-01-19",
  "duration_seconds": 4513,
  "transcript_sha256": "19c7b8e7b557fd720799381854f11c60d4f6b69c684d637b248db082ac914854",
  "transcript_text_chars": 77040,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9988209616662974,
    "text_chars": 77040,
    "completed_at": "2026-06-29 12:27:40"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:1em64iUFt3U
```

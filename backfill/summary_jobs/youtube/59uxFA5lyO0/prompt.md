...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/59uxFA5lyO0/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/59uxFA5lyO0/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/59uxFA5lyO0/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:59uxFA5lyO0",
  "platform": "youtube",
  "platform_id": "59uxFA5lyO0",
  "source_id": "yt_the_daily",
  "source_name": "The Daily",
  "category": "新闻 / 时评 / 全球议题",
  "title": "The Protein Boom: How America Became Obsessed",
  "url": "https://www.youtube.com/watch?v=59uxFA5lyO0",
  "published_at": "2025-08-26T14:45:00+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/59uxFA5lyO0/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-27",
  "duration_seconds": 1577,
  "transcript_sha256": "4d9ddd0ea4eddfffeea6964920b7e41d3f4a3b23a79ad15d704c8f3ee3327b92",
  "transcript_text_chars": 23874,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9957704502219403,
    "text_chars": 23874,
    "completed_at": "2026-06-29 09:34:53"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:59uxFA5lyO0
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/lISewNerBlA/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/lISewNerBlA/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/lISewNerBlA/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:lISewNerBlA",
  "platform": "youtube",
  "platform_id": "lISewNerBlA",
  "source_id": "yt_the_daily",
  "source_name": "The Daily",
  "category": "新闻 / 时评 / 全球议题",
  "title": "Threats and Cash: How China Meddles in U.S. Local Elections",
  "url": "https://www.youtube.com/watch?v=lISewNerBlA",
  "published_at": "2025-08-28T14:18:40+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/lISewNerBlA/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-29",
  "duration_seconds": 1879,
  "transcript_sha256": "14563b72abd1aae7b260f33faaa909e9933da031eace91a520d3ea87d414ab35",
  "transcript_text_chars": 27385,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9985470995210218,
    "text_chars": 27385,
    "completed_at": "2026-06-29 09:39:07"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:lISewNerBlA
```

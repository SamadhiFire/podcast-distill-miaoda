...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/YTVSwOY19Qs/transcript.txt`
- 写入结果文件：`backfill/items/youtube/YTVSwOY19Qs/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:YTVSwOY19Qs",
  "platform": "youtube",
  "platform_id": "YTVSwOY19Qs",
  ...
  "published_at": "2026-04-02T12:00:52+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/YTVSwOY19Qs/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-04-03",
  "duration_seconds": 3822,
  "transcript_sha256": "7e8a48c6d9f80afdabb085ea39fc706da895945e49ecb1acca9c3a064dfdc437",
  "transcript_text_chars": 59132,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9973207744636317,
    "text_chars": 59132,
    "completed_at": "2026-06-29 13:17:26"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:YTVSwOY19Qs
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/m8p5hGQljOQ/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/m8p5hGQljOQ/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/m8p5hGQljOQ/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:m8p5hGQljOQ",
  "platform": "youtube",
  "platform_id": "m8p5hGQljOQ",
  "source_id": "yt_business_breakdowns",
  "source_name": "Business Breakdowns",
  "category": "商业 / 财经 / 投资",
  "title": "Agilent: Back To The Lab - [Business Breakdowns, EP.223]",
  "url": "https://www.youtube.com/watch?v=m8p5hGQljOQ",
  "published_at": "2025-08-17T21:00:42+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/m8p5hGQljOQ/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-18",
  "duration_seconds": 2492,
  "transcript_sha256": "a2d6a6c0ba06924f940d30f77a7d0dc2a6865ea52c8122efd806e9ab24c319db",
  "transcript_text_chars": 44899,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9980939004815409,
    "text_chars": 44899,
    "completed_at": "2026-06-29 09:03:43"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:m8p5hGQljOQ
```

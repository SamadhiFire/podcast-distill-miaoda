...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/y3yAVZk3tyA/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/y3yAVZk3tyA/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/y3yAVZk3tyA/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:y3yAVZk3tyA",
  "platform": "youtube",
  "platform_id": "y3yAVZk3tyA",
  ...
  "published_at": "2025-08-13T20:35:35+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/y3yAVZk3tyA/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-14",
  "duration_seconds": 6570,
  "transcript_sha256": "eb0e4830951dea5a02976c677807740320bc045e758c87cb234d31613d00d694",
  "transcript_text_chars": 95346,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9983438356164384,
    "text_chars": 95346,
    "completed_at": "2026-06-29 08:51:29"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:y3yAVZk3tyA
```

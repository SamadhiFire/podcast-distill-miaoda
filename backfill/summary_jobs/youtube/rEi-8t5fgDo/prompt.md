...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/rEi-8t5fgDo/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/rEi-8t5fgDo/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/rEi-8t5fgDo/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:rEi-8t5fgDo",
  "platform": "youtube",
  "platform_id": "rEi-8t5fgDo",
  "source_id": "yt_bloomberg_china",
  "source_name": "Bloomberg The China Show",
  "category": "商业 / 财经 / 投资",
  "title": "Is China's Stock Rally Getting Too Hot? | The China Show 8/26/2025",
  "url": "https://www.youtube.com/watch?v=rEi-8t5fgDo",
  "published_at": "2025-08-26T06:07:48+00:00",
  "report_date": "2025-08-27",
  "duration_seconds": 5744,
  "language": "en"
}
```
...
把下面 JSON 写入 `backfill/items/youtube/rEi-8t5fgDo/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-27",
  "duration_seconds": 5744,
  "transcript_sha256": "6540f30abdc27d0c486d2fd1e7b37758114575f491cf1a1356f3dcf61a1c94e3",
  "transcript_text_chars": 96998,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.999366295264624,
    "text_chars": 96998,
    "completed_at": "2026-06-29 09:31:19"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:rEi-8t5fgDo
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/qImgGtnNbx0/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/qImgGtnNbx0/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/qImgGtnNbx0/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:qImgGtnNbx0",
  "platform": "youtube",
  "platform_id": "qImgGtnNbx0",
  ...
  "published_at": "2025-07-31T11:01:21+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/qImgGtnNbx0/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-01",
  "duration_seconds": 5338,
  "transcript_sha256": "7266534ff2d2c60497d3751d8123eeb085973ca17fe38db6c85b1c6908214c32",
  "transcript_text_chars": 93857,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9989359310603222,
    "text_chars": 93857,
    "completed_at": "2026-06-29 08:23:42"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:qImgGtnNbx0
```

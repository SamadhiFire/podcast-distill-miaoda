...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/41TiFVsp0AI/transcript.txt`
- 写入结果文件：`backfill/items/youtube/41TiFVsp0AI/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:41TiFVsp0AI",
  "platform": "youtube",
  "platform_id": "41TiFVsp0AI",
  "source_id": "yt_the_daily",
  "source_name": "The Daily",
  "category": "新闻 / 时评 / 全球议题",
  "title": "The Liberal Justices Aren’t as United as You Might Think",
  "url": "https://www.youtube.com/watch?v=41TiFVsp0AI",
  "published_at": "2025-12-10T15:27:59+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/41TiFVsp0AI/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-12-11",
  "duration_seconds": 1996,
  "transcript_sha256": "dc9beedc495f2edb2288ad75905fed62c12f33a29625e98d693f31f0055a275b",
  "transcript_text_chars": 26436,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9969498997995992,
    "text_chars": 26436,
    "completed_at": "2026-06-29 12:03:54"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:41TiFVsp0AI
```

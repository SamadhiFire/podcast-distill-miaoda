...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/yco9JP0PyLM/transcript.txt`
- 写入结果文件：`backfill/items/youtube/yco9JP0PyLM/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:yco9JP0PyLM",
  "platform": "youtube",
  "platform_id": "yco9JP0PyLM",
  ...
  "published_at": "2026-01-08T10:01:40+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/yco9JP0PyLM/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-01-09",
  "duration_seconds": 3879,
  "transcript_sha256": "4a34ed0ff3e730432033dbba50a29605d9032a0466bc38b56860f2318cc3f48a",
  "transcript_text_chars": 74291,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9974526939932973,
    "text_chars": 74291,
    "completed_at": "2026-06-29 12:21:03"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:yco9JP0PyLM
```

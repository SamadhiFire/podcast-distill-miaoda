...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/KtzGVyqiRow/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/KtzGVyqiRow/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/KtzGVyqiRow/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:KtzGVyqiRow",
  "platform": "youtube",
  "platform_id": "KtzGVyqiRow",
  "source_id": "yt_odd_lots_audio",
  "source_name": "Odd Lots Audio",
  "category": "商业 / 财经 / 投资",
  "title": "How to Make Money Selling Pizza in New York City | Odd Lots",
  "url": "https://www.youtube.com/watch?v=KtzGVyqiRow",
  "published_at": "2025-08-04T15:35:38+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/KtzGVyqiRow/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-05",
  "duration_seconds": 2966,
  "transcript_sha256": "58896da8b4c8a042621f502a39d52c1c9efa8dee425024db999cb37f65189f5b",
  "transcript_text_chars": 53446,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9985805799055968,
    "text_chars": 53446,
    "completed_at": "2026-06-29 08:28:56"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:KtzGVyqiRow
```

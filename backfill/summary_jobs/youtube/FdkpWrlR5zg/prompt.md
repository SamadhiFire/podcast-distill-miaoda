...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/FdkpWrlR5zg/transcript.txt`
- 写入结果文件：`backfill/items/youtube/FdkpWrlR5zg/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
把下面 JSON 写入 `backfill/items/youtube/FdkpWrlR5zg/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-12-20",
  "duration_seconds": 6895,
  "transcript_sha256": "28d291503f39e2361c7e7a03d93f97a278c8374a23e06ee448367a3503c139e4",
  "transcript_text_chars": 104220,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9995532994923858,
    "text_chars": 104220,
    "completed_at": "2026-06-29 12:12:03"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:FdkpWrlR5zg
```

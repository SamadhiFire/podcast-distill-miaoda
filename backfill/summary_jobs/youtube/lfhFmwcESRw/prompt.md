...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/lfhFmwcESRw/transcript.txt`
- 写入结果文件：`backfill/items/youtube/lfhFmwcESRw/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:lfhFmwcESRw",
  "platform": "youtube",
  "platform_id": "lfhFmwcESRw",
  ...
  "published_at": "2026-06-16T12:00:11+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/lfhFmwcESRw/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-06-17",
  "duration_seconds": 2326,
  "transcript_sha256": "2d8c547bd847b80d53569f9e225fca37f56f47d751bc88548f39bfe650475795",
  "transcript_text_chars": 38942,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9995356835769562,
    "text_chars": 38942,
    "completed_at": "2026-06-29 14:12:53"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:lfhFmwcESRw
```

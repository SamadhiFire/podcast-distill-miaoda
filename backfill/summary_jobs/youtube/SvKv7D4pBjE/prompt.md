...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/SvKv7D4pBjE/transcript.txt`
- 写入结果文件：`backfill/items/youtube/SvKv7D4pBjE/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:SvKv7D4pBjE",
  "platform": "youtube",
  "platform_id": "SvKv7D4pBjE",
  ...
  "published_at": "2025-09-19T18:17:48+00:00",
  "report_date": "2025-09-20",
  "duration_seconds": 15945,
  "language": "en"
}
```
...
把下面 JSON 写入 `backfill/items/youtube/SvKv7D4pBjE/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-09-20",
  "duration_seconds": 15945,
  "transcript_sha256": "0192581acf757e9b206f602a78961381a454878fac8719a029feae5c71d7c670",
  "transcript_text_chars": 238258,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9993201003449357,
    "text_chars": 238258,
    "completed_at": "2026-06-29 10:28:12"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:SvKv7D4pBjE
```

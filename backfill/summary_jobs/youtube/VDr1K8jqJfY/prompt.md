...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/VDr1K8jqJfY/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/VDr1K8jqJfY/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/VDr1K8jqJfY/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:VDr1K8jqJfY",
  "platform": "youtube",
  "platform_id": "VDr1K8jqJfY",
  "source_id": "yt_bloomberg_china",
  "source_name": "Bloomberg The China Show",
  "category": "商业 / 财经 / 投资",
  "title": "Stock Bulls Take Charge in China | Bloomberg: The China Show 8/14/2025",
  "url": "https://www.youtube.com/watch?v=VDr1K8jqJfY",
  "published_at": "2025-08-14T06:15:11+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/VDr1K8jqJfY/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-15",
  "duration_seconds": 5484,
  "transcript_sha256": "b9ab1f5154cc1cb289971c33713cf6ddb2c0a92d5f446df717ab96718cfb8f41",
  "transcript_text_chars": 93240,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9995331874544128,
    "text_chars": 93240,
    "completed_at": "2026-06-29 08:53:05"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:VDr1K8jqJfY
```

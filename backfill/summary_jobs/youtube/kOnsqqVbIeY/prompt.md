...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/kOnsqqVbIeY/transcript.txt`
- 写入结果文件：`backfill/items/youtube/kOnsqqVbIeY/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:kOnsqqVbIeY",
  "platform": "youtube",
  "platform_id": "kOnsqqVbIeY",
  ...
  "published_at": "2025-10-10T23:28:26+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/kOnsqqVbIeY/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-10-12",
  "duration_seconds": 4898,
  "transcript_sha256": "6ff7f0e5cb5f7ce6adda54da1cb30bdd8106403aa76725f8dc44ddc3095b3ad1",
  "transcript_text_chars": 94961,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9988893425888118,
    "text_chars": 94961,
    "completed_at": "2026-06-29 11:15:12"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:kOnsqqVbIeY
```

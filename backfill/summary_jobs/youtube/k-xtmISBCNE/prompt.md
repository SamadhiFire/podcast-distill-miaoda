...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/k-xtmISBCNE/transcript.txt`
- 写入结果文件：`backfill/items/youtube/k-xtmISBCNE/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:k-xtmISBCNE",
  "platform": "youtube",
  "platform_id": "k-xtmISBCNE",
  ...
  "published_at": "2026-01-08T11:01:32+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/k-xtmISBCNE/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-01-09",
  "duration_seconds": 4581,
  "transcript_sha256": "f7c5e6cc0e27e7413620b7549cc7a43f42b42cbaf3317d807c71dfdc791afd92",
  "transcript_text_chars": 76841,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9990307793058285,
    "text_chars": 76841,
    "completed_at": "2026-06-29 12:21:10"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:k-xtmISBCNE
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/m6qp7a7Mn9I/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/m6qp7a7Mn9I/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/m6qp7a7Mn9I/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:m6qp7a7Mn9I",
  "platform": "youtube",
  "platform_id": "m6qp7a7Mn9I",
  "source_id": "yt_the_daily",
  "source_name": "The Daily",
  "category": "新闻 / 时评 / 全球议题",
  "title": "Trump Said Family Separations Would End. They’re Happening Again.",
  "url": "https://www.youtube.com/watch?v=m6qp7a7Mn9I",
  "published_at": "2025-08-07T14:29:02+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/m6qp7a7Mn9I/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-08",
  "duration_seconds": 1700,
  "transcript_sha256": "27e3bc1f2b23816c026165a9bd3e102555414b424e43f8d2f06ce90fe9006baa",
  "transcript_text_chars": 19672,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9965882352941177,
    "text_chars": 19672,
    "completed_at": "2026-06-29 08:40:08"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:m6qp7a7Mn9I
```

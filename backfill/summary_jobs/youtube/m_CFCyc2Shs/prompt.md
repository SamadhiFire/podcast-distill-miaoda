...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/m_CFCyc2Shs/transcript.txt`
- 写入结果文件：`backfill/items/youtube/m_CFCyc2Shs/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:m_CFCyc2Shs",
  "platform": "youtube",
  "platform_id": "m_CFCyc2Shs",
  ...
  "published_at": "2025-11-17T18:27:43+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/m_CFCyc2Shs/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-11-18",
  "duration_seconds": 9415,
  "transcript_sha256": "720ce50f9b80ea68cd12d8ffa0516984e9e7299cf7d1843986299bfa0481a854",
  "transcript_text_chars": 148016,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.998721189591078,
    "text_chars": 148016,
    "completed_at": "2026-06-29 11:47:10"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:m_CFCyc2Shs
```

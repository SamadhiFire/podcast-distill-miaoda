...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/hUJz55AsUz4/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/hUJz55AsUz4/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/hUJz55AsUz4/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:hUJz55AsUz4",
  "platform": "youtube",
  "platform_id": "hUJz55AsUz4",
  "source_id": "yt_bg2pod",
  "source_name": "Bg2 Pod",
  "category": "科技 / AI / VC",
  "title": "China, China, China. Breaking Down China’s Tech Surge | BG2 w/ Bill Gurley and Brad Gerstner",
  "url": "https://www.youtube.com/watch?v=hUJz55AsUz4",
  "published_at": "2025-08-28T23:51:07+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/hUJz55AsUz4/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-30",
  "duration_seconds": 3993,
  "transcript_sha256": "c1377fdb6b4d3092d04cfba1a13437fcfdf3064f365da2136b4fab0e6cdb1568",
  "transcript_text_chars": 61041,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9993285750062609,
    "text_chars": 61041,
    "completed_at": "2026-06-29 09:40:29"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:hUJz55AsUz4
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/nepKKz-MzFM/transcript.txt`
- 写入结果文件：`backfill/items/youtube/nepKKz-MzFM/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:nepKKz-MzFM",
  "platform": "youtube",
  "platform_id": "nepKKz-MzFM",
  ...
  "published_at": "2026-05-06T22:03:16+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/nepKKz-MzFM/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-05-08",
  "duration_seconds": 15502,
  "transcript_sha256": "492af225968c05ffcd68ca7163e029e63a1db543594b23acaefccd16dfd95dce",
  "transcript_text_chars": 238380,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9992233260224487,
    "text_chars": 238380,
    "completed_at": "2026-06-29 13:43:03"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:nepKKz-MzFM
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/_Y5xAZKIGU0/transcript.txt`
- 写入结果文件：`backfill/items/youtube/_Y5xAZKIGU0/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:_Y5xAZKIGU0",
  "platform": "youtube",
  "platform_id": "_Y5xAZKIGU0",
  ...
  "published_at": "2026-02-19T09:09:08+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/_Y5xAZKIGU0/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-02-20",
  "duration_seconds": 3037,
  "transcript_sha256": "f026fd7e6cc1af6996744537eec5112b836c34ef016fb147cbda7e13c190e7ce",
  "transcript_text_chars": 55235,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9957194599934146,
    "text_chars": 55235,
    "completed_at": "2026-06-29 12:48:32"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:_Y5xAZKIGU0
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/1qlD_a3mXR0/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/1qlD_a3mXR0/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/1qlD_a3mXR0/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:1qlD_a3mXR0",
  "platform": "youtube",
  "platform_id": "1qlD_a3mXR0",
  "source_id": "yt_the_daily",
  "source_name": "The Daily",
  "category": "新闻 / 时评 / 全球议题",
  "title": "A ‘Dagger in the Heart’ of Climate Change Regulation",
  "url": "https://www.youtube.com/watch?v=1qlD_a3mXR0",
  "published_at": "2025-07-31T14:19:33+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/1qlD_a3mXR0/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-01",
  "duration_seconds": 1509,
  "transcript_sha256": "4b14f850f72073c81594c35d84bda7ed902a5d0e444ff4322be9de9145bcc3d4",
  "transcript_text_chars": 19437,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9972034459907223,
    "text_chars": 19437,
    "completed_at": "2026-06-29 08:24:25"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:1qlD_a3mXR0
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/QhGqLqCzFac/transcript.txt`
- 写入结果文件：`backfill/items/youtube/QhGqLqCzFac/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:QhGqLqCzFac",
  "platform": "youtube",
  "platform_id": "QhGqLqCzFac",
  ...
  "published_at": "2026-05-16T08:12:55+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/QhGqLqCzFac/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-05-17",
  "duration_seconds": 2975,
  "transcript_sha256": "d982978b1a0453518907971f7c607f2dda11d266477ee0d58e634e822ce99a6d",
  "transcript_text_chars": 52258,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.998693781512605,
    "text_chars": 52258,
    "completed_at": "2026-06-29 13:51:27"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:QhGqLqCzFac
```

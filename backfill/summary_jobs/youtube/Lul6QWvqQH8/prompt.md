...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/Lul6QWvqQH8/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/Lul6QWvqQH8/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/Lul6QWvqQH8/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:Lul6QWvqQH8",
  "platform": "youtube",
  "platform_id": "Lul6QWvqQH8",
  "source_id": "yt_sequoia",
  "source_name": "Sequoia Capital",
  "category": "科技 / AI / VC",
  "title": "How AI Digital Minds Can Scale Human Connection ft Delphi’s Dara Ladjevardian",
  "url": "https://www.youtube.com/watch?v=Lul6QWvqQH8",
  "published_at": "2025-08-12T09:00:37+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/Lul6QWvqQH8/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-13",
  "duration_seconds": 2346,
  "transcript_sha256": "28b36cb9d2b7932121e2ba3b95278dcdc45ecbe88036009098e9a90f2357d5e8",
  "transcript_text_chars": 42477,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9941389599317988,
    "text_chars": 42477,
    "completed_at": "2026-06-29 08:48:38"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:Lul6QWvqQH8
```

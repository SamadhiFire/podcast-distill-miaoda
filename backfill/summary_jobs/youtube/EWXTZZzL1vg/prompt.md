...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/EWXTZZzL1vg/transcript.txt`
- 写入结果文件：`backfill/items/youtube/EWXTZZzL1vg/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:EWXTZZzL1vg",
  "platform": "youtube",
  "platform_id": "EWXTZZzL1vg",
  ...
  "url": "https://www.youtube.com/watch?v=EWXTZZzL1vg",
  "published_at": "2025-07-01T09:00:18+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/EWXTZZzL1vg/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-07-02",
  "duration_seconds": 3578,
  "transcript_sha256": "51abb591eefbfe57d3d27034b8fc0b5737dfebf516078d45bdcf120b57144838",
  "transcript_text_chars": 58276,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9961263275572946,
    "text_chars": 58276,
    "completed_at": "2026-06-29 07:29:12"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:EWXTZZzL1vg
```

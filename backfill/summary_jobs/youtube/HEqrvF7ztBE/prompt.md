...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/HEqrvF7ztBE/transcript.txt`
- 写入结果文件：`backfill/items/youtube/HEqrvF7ztBE/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:HEqrvF7ztBE",
  "platform": "youtube",
  "platform_id": "HEqrvF7ztBE",
  ...
  "url": "https://www.youtube.com/watch?v=HEqrvF7ztBE",
  "published_at": "2026-03-12T12:30:20+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/HEqrvF7ztBE/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-03-13",
  "duration_seconds": 4014,
  "transcript_sha256": "1684379294a4bdcf589b7c3e9fedee94f265c1b6315100f0c6656be6492effd5",
  "transcript_text_chars": 74930,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9989935226706528,
    "text_chars": 74930,
    "completed_at": "2026-06-29 13:02:30"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:HEqrvF7ztBE
```

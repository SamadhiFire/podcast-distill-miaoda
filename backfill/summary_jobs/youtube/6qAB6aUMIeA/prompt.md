...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/6qAB6aUMIeA/transcript.txt`
- 写入结果文件：`backfill/items/youtube/6qAB6aUMIeA/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:6qAB6aUMIeA",
  "platform": "youtube",
  "platform_id": "6qAB6aUMIeA",
  ...
  "published_at": "2025-12-18T13:30:05+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/6qAB6aUMIeA/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-12-19",
  "duration_seconds": 5516,
  "transcript_sha256": "fadf9bda395cb9c3b9fb77cc69a75d8f6d403cddec8f157876d05392f9636168",
  "transcript_text_chars": 94693,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9989702683103698,
    "text_chars": 94693,
    "completed_at": "2026-06-29 12:10:51"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:6qAB6aUMIeA
```

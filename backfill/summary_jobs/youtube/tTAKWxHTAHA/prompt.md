...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/tTAKWxHTAHA/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/tTAKWxHTAHA/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/tTAKWxHTAHA/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:tTAKWxHTAHA",
  "platform": "youtube",
  "platform_id": "tTAKWxHTAHA",
  "source_id": "yt_cannonball",
  "source_name": "Cannonball with Wesley Morris",
  "category": "文化 / 社会 / 人文",
  "title": "Spike Lee's ‘Highest 2 Lowest’ Gives Us Black Maximalism And Denzel’s Best Hairline Yet | Cannonball",
  "url": "https://www.youtube.com/watch?v=tTAKWxHTAHA",
  "published_at": "2025-08-21T13:01:38+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/tTAKWxHTAHA/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-22",
  "duration_seconds": 2958,
  "transcript_sha256": "13ca5179b7e39011a7e6944035c7fbc52c2d7eb0f4317b1376222a837f43ab3b",
  "transcript_text_chars": 43317,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9943238674780257,
    "text_chars": 43317,
    "completed_at": "2026-06-29 09:22:53"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:tTAKWxHTAHA
```

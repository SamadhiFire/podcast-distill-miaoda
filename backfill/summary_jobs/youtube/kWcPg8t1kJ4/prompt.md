...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/kWcPg8t1kJ4/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/kWcPg8t1kJ4/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/kWcPg8t1kJ4/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:kWcPg8t1kJ4",
  "platform": "youtube",
  "platform_id": "kWcPg8t1kJ4",
  ...
  "published_at": "2025-08-07T16:13:32+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/kWcPg8t1kJ4/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-08",
  "duration_seconds": 4085,
  "transcript_sha256": "69777b749470d40cb0971f96898fb93643084701d9e5692f84da9335ddd35e2c",
  "transcript_text_chars": 75886,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9996570379436965,
    "text_chars": 75886,
    "completed_at": "2026-06-29 08:41:12"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:kWcPg8t1kJ4
```

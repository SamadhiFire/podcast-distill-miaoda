...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/Ctjiatnd6Xk/transcript.txt`
- 写入结果文件：`backfill/items/youtube/Ctjiatnd6Xk/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:Ctjiatnd6Xk",
  "platform": "youtube",
  "platform_id": "Ctjiatnd6Xk",
  ...
  "published_at": "2025-11-16T14:00:20+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/Ctjiatnd6Xk/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-11-17",
  "duration_seconds": 4774,
  "transcript_sha256": "3ada484cee474209408d07e6379774a1f59773a81c32ef35eb784a1435a771bd",
  "transcript_text_chars": 64227,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9989777963971512,
    "text_chars": 64227,
    "completed_at": "2026-06-29 11:45:51"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:Ctjiatnd6Xk
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/48pxVdmkMIE/transcript.txt`
- 写入结果文件：`backfill/items/youtube/48pxVdmkMIE/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:48pxVdmkMIE",
  "platform": "youtube",
  "platform_id": "48pxVdmkMIE",
  ...
  "published_at": "2025-09-12T15:32:21+00:00",
  "report_date": "2025-09-13",
  "duration_seconds": 5309,
  "language": "en"
}
```
...
把下面 JSON 写入 `backfill/items/youtube/48pxVdmkMIE/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-09-13",
  "duration_seconds": 5309,
  "transcript_sha256": "bbccd4a114dea8c5002b9a986d044eb4cf9d3eb060e774a8b76455c9943c28fa",
  "transcript_text_chars": 97776,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9989677905443587,
    "text_chars": 97776,
    "completed_at": "2026-06-29 10:09:57"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:48pxVdmkMIE
```

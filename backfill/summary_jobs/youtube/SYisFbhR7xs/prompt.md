...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/SYisFbhR7xs/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/SYisFbhR7xs/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/SYisFbhR7xs/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:SYisFbhR7xs",
  "platform": "youtube",
  "platform_id": "SYisFbhR7xs",
  ...
  "url": "https://www.youtube.com/watch?v=SYisFbhR7xs",
  "published_at": "2025-08-21T10:00:47+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/SYisFbhR7xs/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-22",
  "duration_seconds": 2532,
  "transcript_sha256": "7c560d27483d583aec318fb67edcdafe5ddfb5e62cbdebadf203880f5e4a5eca",
  "transcript_text_chars": 48249,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9983566350710901,
    "text_chars": 48249,
    "completed_at": "2026-06-29 09:22:09"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:SYisFbhR7xs
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/qbvY0dQgSJ4/transcript.txt`
- 写入结果文件：`backfill/items/youtube/qbvY0dQgSJ4/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:qbvY0dQgSJ4",
  "platform": "youtube",
  "platform_id": "qbvY0dQgSJ4",
  ...
  "published_at": "2025-10-23T11:01:38+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/qbvY0dQgSJ4/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-10-24",
  "duration_seconds": 4956,
  "transcript_sha256": "d57b385436afd69dd587f5bb936dacec940fa8a721ce3ec2d1d4ab970a4e3156",
  "transcript_text_chars": 87241,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9989344229217111,
    "text_chars": 87241,
    "completed_at": "2026-06-29 11:24:50"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:qbvY0dQgSJ4
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/SlGRN8jh2RI/transcript.txt`
- 写入结果文件：`backfill/items/youtube/SlGRN8jh2RI/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:SlGRN8jh2RI",
  "platform": "youtube",
  "platform_id": "SlGRN8jh2RI",
  ...
  "published_at": "2026-05-04T17:28:44+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/SlGRN8jh2RI/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-05-05",
  "duration_seconds": 1476,
  "transcript_sha256": "756c17c9dcc2e1a3e1e026cd925ee19619650a462374ad493f71c4656e4bba3e",
  "transcript_text_chars": 27245,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9979403794037941,
    "text_chars": 27245,
    "completed_at": "2026-06-29 13:40:22"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:SlGRN8jh2RI
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/4hdkgfjUkfM/transcript.txt`
- 写入结果文件：`backfill/items/youtube/4hdkgfjUkfM/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:4hdkgfjUkfM",
  "platform": "youtube",
  "platform_id": "4hdkgfjUkfM",
  ...
  "published_at": "2025-12-18T18:30:11+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/4hdkgfjUkfM/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-12-19",
  "duration_seconds": 1766,
  "transcript_sha256": "8fdaf706c1b79a7b72d966c29dfbba3f9b7c8cc96fc3f7a54bd803062edbc799",
  "transcript_text_chars": 26044,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9995469988674972,
    "text_chars": 26044,
    "completed_at": "2026-06-29 12:11:16"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:4hdkgfjUkfM
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/s5wdJqJ708U/transcript.txt`
- 写入结果文件：`backfill/items/youtube/s5wdJqJ708U/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:s5wdJqJ708U",
  "platform": "youtube",
  "platform_id": "s5wdJqJ708U",
  ...
  "published_at": "2025-11-14T15:01:58+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/s5wdJqJ708U/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-11-15",
  "duration_seconds": 2888,
  "transcript_sha256": "26c6a8477589142c2598e24928db03eac9b0a3c1440070163710dd3ce82bd360",
  "transcript_text_chars": 50556,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9988410664819943,
    "text_chars": 50556,
    "completed_at": "2026-06-29 11:44:49"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:s5wdJqJ708U
```

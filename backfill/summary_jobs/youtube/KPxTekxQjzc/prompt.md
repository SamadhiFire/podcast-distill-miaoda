...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/KPxTekxQjzc/transcript.txt`
- 写入结果文件：`backfill/items/youtube/KPxTekxQjzc/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:KPxTekxQjzc",
  "platform": "youtube",
  "platform_id": "KPxTekxQjzc",
  ...
  "published_at": "2025-09-11T11:00:37+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/KPxTekxQjzc/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-09-12",
  "duration_seconds": 5880,
  "transcript_sha256": "71faa852a20522ae95f7c30ba995f9544a136d04a1a1962c8a4ae53f91b66ec2",
  "transcript_text_chars": 90418,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9990882653061225,
    "text_chars": 90418,
    "completed_at": "2026-06-29 10:03:43"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:KPxTekxQjzc
```

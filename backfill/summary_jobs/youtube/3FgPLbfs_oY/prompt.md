...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/3FgPLbfs_oY/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/3FgPLbfs_oY/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/3FgPLbfs_oY/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:3FgPLbfs_oY",
  "platform": "youtube",
  "platform_id": "3FgPLbfs_oY",
  ...
  "published_at": "2025-08-29T17:00:10+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/3FgPLbfs_oY/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-30",
  "duration_seconds": 2279,
  "transcript_sha256": "a15048d501539d143d252b528b4bd450b615086901b7284c4aff9713f0c3c5f7",
  "transcript_text_chars": 38128,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9992799473453269,
    "text_chars": 38128,
    "completed_at": "2026-06-29 09:42:29"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:3FgPLbfs_oY
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/ZNzYN2jyVTU/transcript.txt`
- 写入结果文件：`backfill/items/youtube/ZNzYN2jyVTU/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:ZNzYN2jyVTU",
  "platform": "youtube",
  "platform_id": "ZNzYN2jyVTU",
  ...
  "published_at": "2026-05-06T15:24:37+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/ZNzYN2jyVTU/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2026-05-07",
  "duration_seconds": 1576,
  "transcript_sha256": "304c7b92f34e47764ac29273b42d9d058a31d7381a47cb072e3f9135892c51c5",
  "transcript_text_chars": 27226,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9972335025380711,
    "text_chars": 27226,
    "completed_at": "2026-06-29 13:41:42"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:ZNzYN2jyVTU
```

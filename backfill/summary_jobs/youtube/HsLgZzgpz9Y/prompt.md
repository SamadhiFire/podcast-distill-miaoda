...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/HsLgZzgpz9Y/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/HsLgZzgpz9Y/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/HsLgZzgpz9Y/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:HsLgZzgpz9Y",
  "platform": "youtube",
  "platform_id": "HsLgZzgpz9Y",
  ...
  "published_at": "2025-08-29T23:52:25+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/HsLgZzgpz9Y/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-31",
  "duration_seconds": 6629,
  "transcript_sha256": "ab8449aef4c5e4d5c17393e6590af311522005cbca7ee76d038468bf9d506358",
  "transcript_text_chars": 122533,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9984251018253131,
    "text_chars": 122533,
    "completed_at": "2026-06-29 09:42:52"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:HsLgZzgpz9Y
```

...
- 只处理这一条 YouTube 节目。
- 读取完整字幕：`backfill/items/youtube/6jyV-9D9cKM/transcript.txt`
- 如需核对逐字引用或时间上下文，可读取 VTT：`backfill/items/youtube/6jyV-9D9cKM/transcript.vtt`
- 写入结果文件：`backfill/items/youtube/6jyV-9D9cKM/summary.json`
- 不修改 SQLite。
- 不发布飞书。
- 不处理小宇宙。
- 不改动阶段 D 进程或状态。
...
```json
{
  "item_id": "youtube:6jyV-9D9cKM",
  "platform": "youtube",
  "platform_id": "6jyV-9D9cKM",
  "source_id": "yt_wsj_take_on_week",
  "source_name": "WSJ Take On the Week",
  "category": "新闻 / 时评 / 全球议题",
  "title": "Is the Market in a Speculative Bubble? T. Rowe Price CIO Weighs In | WSJ's Take On the Week",
  "url": "https://www.youtube.com/watch?v=6jyV-9D9cKM",
  "published_at": "2025-08-03T10:00:08+00:00",
...
把下面 JSON 写入 `backfill/items/youtube/6jyV-9D9cKM/summary.json`。字段名必须保持一致。`digest` 必须完全基于字幕内容，不要编造字幕中没有的信息。
...
```json
{
  ...
  "report_date": "2025-08-04",
  "duration_seconds": 1395,
  "transcript_sha256": "bfa6797922b1b6c0bcf7b407573febc0021325a85527a14eb19dc74884a0af19",
  "transcript_text_chars": 21621,
  "extraction": {
    "method": "yt-dlp",
    "language": "en",
    "coverage_ratio": 0.9945591397849463,
    "text_chars": 21621,
    "completed_at": "2026-06-29 08:27:24"
  },
  "generation": {
    "script": "trae_agent",
...
```powershell
python scripts\backfill\validate_summary_outputs.py --platform youtube --item-id youtube:6jyV-9D9cKM
```
